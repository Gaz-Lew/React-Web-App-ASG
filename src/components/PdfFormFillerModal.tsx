...
import {
  X, FileText, Download, Save, Loader, Search, Pen, Camera, Plus,
  CalendarDays, UserPlus, MousePointerClick, Trash2, Pencil,
} from 'lucide-react';
...
interface AttachedPhoto {
  id: string; dataUrl: string; name: string; mimeType: string;
}
...
function detectAcroType(fieldName: string): CustomFieldType {
  ...
  if (n.includes('price') || n.includes('cost') || n.includes('amount') || n.includes('total')
    || n.includes('fee') || n.includes('value') || n.includes('payment') || n.includes('dollar')) return 'currency';
...
function formatForPdf(value: string, type: CustomFieldType): string {
  ...
  switch (type) {
    ...
    case 'time': return value; // HH:MM — fine as-is
...
function SignatureCanvas({ label, onSign }: { label: string; onSign: (d: string | null) => void }) {
  ...
  const onMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    ctx.stroke(); hasSig.current = true;
  };
  ...
  const clear = () => {
    ...
    hasSig.current = false; onSign(null);
  };
  useEffect(() => {
    const c = canvasRef.current!; const ctx = c.getContext('2d')!;
    ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, c.width, c.height);
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button onClick={clear} className="text-xs text-gray-400 hover:text-red-500 transition px-2 py-0.5 rounded">✕ Clear</button>
      </div>
      <canvas ref={canvasRef} width={1200} height={280}
...
export function PdfFormFillerModal({
  template, onClose,
}: {
  ...
  const [rubberBand, setRubberBand] = useState<{ pageIndex: number; left: number; top: number; width: number; height: number } | null>(null);
  ...
  const templateSnap = useRef({ pdfUrl: template.pdfUrl, pdfStoragePath: template.pdfStoragePath, name: template.name });
  ...
  useEffect(() => {
    fetch('/asg-logo.png')
      ...
      .then((buf) => { logoBytesRef.current = new Uint8Array(buf); })
  ...
  useEffect(() => {
    ...
    if (!pdfUrl) { setLoadError('No PDF URL found on this template.'); setLoading(false); return; }
    let cancelled = false;
      ...
      try {
        ...
        for (let i = 1; i <= doc.numPages; i++) {
          ...
          for (const a of anns) {
            ...
            fields.push({
              fieldName: a.fieldName, fieldType: a.fieldType ?? 'Tx',
              multiLine: a.multiLine ?? false,
              isCheckbox: a.fieldType === 'Btn' && !a.radioButton,
              options: (a.options ?? []).map((o: any) => typeof o === 'string' ? o : (o.displayValue ?? o.exportValue ?? String(o))),
              rect: { left: Math.min(vr[0], vr[2]), top: Math.min(vr[1], vr[3]), width: Math.abs(vr[2] - vr[0]), height: Math.abs(vr[3] - vr[1]) },
            });
          }
          pageResults.push({ dataUrl: canvas.toDataURL('image/jpeg', 0.88), width: canvas.width, height: canvas.height, fields });
        }

        if (!cancelled) {
          setPages(pageResults);
          const init: Record<string, string | boolean> = {};
          pageResults.forEach((p) => p.fields.forEach((f) => { if (!(f.fieldName in init)) init[f.fieldName] = f.isCheckbox ? false : ''; }));
          setFieldValues(init);
          setFilename(`${name} ${new Date().toLocaleDateString('en-AU').replace(/\//g, '-')}`);
          setLoading(false);
        }
      } catch (err) {
        if (!cancelled) { setLoadError(err instanceof Error ? err.message : 'Failed to load PDF'); setLoading(false); }
      }
    })();
  ...
  const onPagePointerMove = (e: React.PointerEvent<HTMLDivElement>, pageIndex: number) => {
    ...
    setRubberBand({
      pageIndex,
      left:   Math.min(sx, x), top:    Math.min(sy, y),
  ...
  const onPagePointerUp = (e: React.PointerEvent<HTMLDivElement>, pageIndex: number) => {
    ...
    setFieldConfig({ pageIndex, rect: { left: rb.left, top: rb.top, width: rb.width, height: rb.height }, type: 'text', label: '', fontSize: 12, optionsText: '' });
  };
  ...
  const saveFieldConfig = () => {
    ...
    const options = fieldConfig.type === 'dropdown'
      ? fieldConfig.optionsText.split('\n').map((s) => s.trim()).filter(Boolean)
      : [];

    if (fieldConfig.editingId) {
      setCustomFields((prev) => prev.map((f) => f.id === fieldConfig.editingId
      ...
      setCustomFields((prev) => [...prev, { id, pageIndex: fieldConfig.pageIndex, rect: fieldConfig.rect, type: fieldConfig.type, label: fieldConfig.label, fontSize: fieldConfig.fontSize, options }]);
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    Array.from(e.target.files ?? []).forEach((file) => {
      ...
      reader.onload = (ev) => setPhotos((p) => [...p, { id: `ph_${Date.now()}_${Math.random().toString(36).slice(2)}`, dataUrl: ev.target!.result as string, name: file.name, mimeType: file.type || 'image/jpeg' }]);
  ...
  const formattedDate = documentDate
    ? new Date(documentDate + 'T12:00:00').toLocaleDateString('en-AU', { day: '2-digit', month: 'long', year: 'numeric' })
    : '';
  ...
  const fillAndExport = useCallback(async (): Promise<Uint8Array> => {
    ...
    for (const [name, value] of Object.entries(fieldValues)) {
      try {
        ...
        if (field instanceof PDFTextField) {
          ...
          try { field.setFontSize(effectiveSize); } catch { /* ignore */ }
          const autoType = detectAcroType(name);
          field.setText(autoType === 'currency' || autoType === 'date'
            ? formatForPdf(strVal, autoType)
            : strVal);
        } else if (field instanceof PDFCheckBox) {
          value ? field.check() : field.uncheck();
        } else if (field instanceof PDFDropdown && strVal) {
          try { field.select(strVal); } catch { /* option not found */ }
        }
    ...
    const GOLD     = rgb(0.961, 0.620, 0.043);             // amber/gold accent strip
    const DARK     = rgb(0.12, 0.16, 0.24);                // near-black text
    const MID      = rgb(0.40, 0.40, 0.45);                // secondary text
    ...
    const drawHeader = async (
      ...
      titleLine: string, metaLine: string,
    ) => {
      // Background
      ...
      // Meta line
      if (metaLine) page.drawText(metaLine, { x: 16, y: pgH - 40, size: 8.5, font: helvetica, color: MID, maxWidth: pgW - 140 });
    ...
    for (const cf of customFields) {
      ...
      page.drawRectangle({
        x: pdfX, y: pdfYb,
        width: cf.rect.width / RENDER_SCALE, height: cf.rect.height / RENDER_SCALE,
    ...
    if (sig1DataUrl || sig2DataUrl) {
      ...
      const drawSig = async (
        dataUrl: string | null, label: string, x: number, maxW: number, startY: number,
        signerName: string, signerDate: string,
      ) => {
    ...
    return pdfDoc.save();
  }, [fieldValues, acroFontSizes, globalFontSize, customFields, customValues, photos, sig1DataUrl, sig1Name, sig1Date, sig2DataUrl, sig2Name, sig2Date, selectedLead, currentUser, documentTitle, formattedDate]);
  ...
  const handleDownload = () => doExport(async (blob, name) => {
  ...
  const handleSaveToLead = () => {
    ...
    doExport(async (blob, name) => {
      ...
      await addDoc(collection(db, 'leads', String(selectedLead.id), 'files'), {
        name, storagePath, downloadUrl, fileType: 'application/pdf',
        fileSize: blob.size, type: 'document', uploadedBy: currentUser.name, uploadedAt: Date.now(),
      } as Omit<LeadFile, 'id'>);
      const url = URL.createObjectURL(blob);
      Object.assign(document.createElement('a'), { href: url, download: name }).click();
      URL.revokeObjectURL(url);
  ...
  const filteredLeads = leadSearch.trim()
    ? leads.filter((l) => { const t = leadSearch.toLowerCase(); return l.name.toLowerCase().includes(t) || l.phone.includes(t) || l.suburb.toLowerCase().includes(t); }).slice(0, 8)
    : [];
  ...
  const renderAcroField = (field: AcroField, fi: number) => {
    ...
    const resetFs = () => setAcroFontSizes((p) => { const n = { ...p }; delete n[field.fieldName]; return n; });

    // Shared input style — positioned at (0,0) inside the wrapper
    const inp: React.CSSProperties = {
      position: 'absolute', left: 0, top: 0,
      width: field.rect.width, height: field.rect.height,
      border: 'none', outline: '2px solid rgba(245,158,11,0.6)', outlineOffset: '-1px',
      borderRadius: 2, background: 'rgba(255,255,255,0.9)', boxSizing: 'border-box',
      fontFamily: 'Arial, Helvetica, sans-serif', color: '#1e293b', padding: '1px 4px',
    };

    // Checkbox — no font size control needed
    if (field.isCheckbox) return (
      <div key={key} style={{ position: 'absolute', left: field.rect.left, top: field.rect.top, width: field.rect.width, height: field.rect.height, background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <input type="checkbox" checked={!!val}
          onChange={(e) => setFieldValues((p) => ({ ...p, [field.fieldName]: e.target.checked }))}
          style={{ width: Math.min(field.rect.width * 0.7, 22), height: Math.min(field.rect.height * 0.7, 22), cursor: 'pointer' }} />
      </div>
    );
    ...
    const fsPill = (
      <div
        ...
        style={{ position: 'absolute', top: -20, right: 0, display: 'flex', alignItems: 'center', background: '#1e293b', borderRadius: 4, padding: '2px 4px', gap: 2, zIndex: 30, pointerEvents: 'auto' }}
      >
        <button
          onMouseDown={(e) => e.preventDefault()}
          onClick={(e) => { e.stopPropagation(); adjustFs(-1); }}
          style={{ color: '#fff', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, lineHeight: 1, padding: '0 2px', fontWeight: 700 }}>−</button>
        <span style={{ color: hasOverride ? '#f59e0b' : '#94a3b8', fontSize: 10, fontWeight: 700, minWidth: 22, textAlign: 'center' }}>{fs}pt</span>
        <button
          onMouseDown={(e) => e.preventDefault()}
          onClick={(e) => { e.stopPropagation(); adjustFs(1); }}
          style={{ color: '#fff', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, lineHeight: 1, padding: '0 2px', fontWeight: 700 }}>+</button>
        {hasOverride && (
          <button
            onMouseDown={(e) => e.preventDefault()}
            onClick={(e) => { e.stopPropagation(); resetFs(); }}
            title="Reset to global font size"
            style={{ color: '#94a3b8', background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, lineHeight: 1, padding: '0 2px' }}>↺</button>
        )}
      </div>
    );

    const wrap = (input: React.ReactNode) => (
      <div key={key} className="group" style={{ position: 'absolute', left: field.rect.left, top: field.rect.top, width: field.rect.width, height: field.rect.height }}>
        {fsPill}
        {input}
      </div>
    );

    if (field.fieldType === 'Ch' && field.options.length > 0) return wrap(
    ...
    if (autoType === 'date') return wrap(
      <input type="date" value={typeof val === 'string' ? val : ''}
        onChange={(e) => setFieldValues((p) => ({ ...p, [field.fieldName]: e.target.value }))}
        style={{ ...inp, fontSize: Math.min(fs, 12), cursor: 'pointer' }} />
    );

    if (autoType === 'time') return wrap(
      <input type="time" value={typeof val === 'string' ? val : ''}
        onChange={(e) => setFieldValues((p) => ({ ...p, [field.fieldName]: e.target.value }))}
        style={{ ...inp, fontSize: Math.min(fs, 12), cursor: 'pointer' }} />
    );

    if (autoType === 'currency') return wrap(
      <div style={{ ...inp, display: 'flex', alignItems: 'center', padding: '0 4px' }}>
        ...
        <input type="number" min="0" step="0.01" value={typeof val === 'string' ? val : ''}
          onChange={(e) => setFieldValues((p) => ({ ...p, [field.fieldName]: e.target.value }))}
          style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', color: '#1e293b', fontSize: fs, fontFamily: 'inherit' }} />
      </div>
    );

    if (autoType === 'number') return wrap(
      <input type="number" value={typeof val === 'string' ? val : ''}
        onChange={(e) => setFieldValues((p) => ({ ...p, [field.fieldName]: e.target.value }))}
    ...
    return wrap(
      <Tag value={typeof val === 'string' ? val : ''}
        onChange={(e) => setFieldValues((p) => ({ ...p, [field.fieldName]: e.target.value }))}
  ...
  const renderCustomField = (cf: CustomField) => {
    ...
    const base: React.CSSProperties = {
      position: 'absolute', left: cf.rect.left, top: cf.rect.top,
      width: cf.rect.width, height: cf.rect.height,
      border: 'none', outline: '2px solid rgba(59,130,246,0.7)', outlineOffset: '-1px',
      borderRadius: 2, background: 'rgba(239,246,255,0.92)', boxSizing: 'border-box',
      fontFamily: 'Arial, Helvetica, sans-serif', color: '#1e293b', padding: '1px 4px',
    };
    ...
    const controls = (
      <div style={{ position: 'absolute', top: -18, right: 0, display: 'flex', gap: 3, zIndex: 20 }}>
        <button
          ...
          onClick={(e) => { e.stopPropagation(); setFieldConfig({ pageIndex: cf.pageIndex, rect: cf.rect, type: cf.type, label: cf.label, fontSize: cf.fontSize, optionsText: cf.options.join('\n'), editingId: cf.id }); }}
          style={{ background: '#3b82f6', color: '#fff', border: 'none', borderRadius: 3, padding: '1px 4px', cursor: 'pointer', fontSize: 10, display: 'flex', alignItems: 'center', gap: 2 }}>
          <span>✏</span>
        </button>
        <button
          onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); }}
          onClick={(e) => { e.stopPropagation(); setCustomFields((p) => p.filter((f) => f.id !== cf.id)); setCustomValues((p) => { const n = { ...p }; delete n[cf.id]; return n; }); }}
          style={{ background: '#ef4444', color: '#fff', border: 'none', borderRadius: 3, padding: '1px 4px', cursor: 'pointer', fontSize: 10 }}>
          ✕
        </button>
      </div>
    );
    ...
    const wrapper = (input: React.ReactNode) => (
      <div key={cf.id} className="group" style={{ position: 'absolute', left: cf.rect.left, top: cf.rect.top, width: cf.rect.width, height: cf.rect.height }}>
        <div className="opacity-0 group-hover:opacity-100 transition-opacity" style={{ position: 'absolute', top: -18, right: 0 }}>
          {controls}
        </div>
        {input}
      </div>
    ...
    if (cf.type === 'dropdown') return wrapper(
    ...
    if (cf.type === 'date') return wrapper(
      <input type="date" value={val} onChange={(e) => setCustomValues((p) => ({ ...p, [cf.id]: e.target.value }))}
        style={{ ...base, left: 0, top: 0, fontSize: Math.min(fs, 12), cursor: 'pointer' }} />
    );

    if (cf.type === 'time') return wrapper(
      <input type="time" value={val} onChange={(e) => setCustomValues((p) => ({ ...p, [cf.id]: e.target.value }))}
        style={{ ...base, left: 0, top: 0, fontSize: Math.min(fs, 12), cursor: 'pointer' }} />
    );

    if (cf.type === 'currency') return wrapper(
      <div style={{ ...base, left: 0, top: 0, display: 'flex', alignItems: 'center', padding: '0 4px' }}>
        ...
        <input type="number" min="0" step="0.01" value={val}
          ...
          style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', color: '#1e293b', fontSize: Math.min(fs, 13), fontFamily: 'inherit' }} />
      </div>
    );

    if (cf.type === 'number') return wrapper(
    ...
    <>
            ...
            <div className="flex items-center gap-2 px-4 py-2.5 flex-wrap gap-y-2">
              ...
              <div className="flex-1 min-w-40">
                <input value={documentTitle} onChange={(e) => setDocumentTitle(e.target.value)}
                  ...
                  placeholder="Document title…" title="Click to rename" />
              ...
              <div className="flex items-center gap-0.5 flex-shrink-0 bg-gray-100 dark:bg-slate-800 rounded-lg px-2 py-1" title="Default font size for all form fields. Per-field overrides still available on hover.">
                <span className="text-xs text-gray-500 dark:text-gray-400 mr-1">Font:</span>
                <button
                  onMouseDown={(e) => e.preventDefault()}
                  onClick={() => setGlobalFontSize((s) => Math.max(6, s - 1))}
                  className="text-gray-600 dark:text-gray-300 hover:text-amber-500 font-bold text-sm leading-none px-1 transition">−</button>
                <span className="text-xs font-bold text-amber-600 dark:text-amber-400 min-w-[28px] text-center">{globalFontSize}pt</span>
                <button
                  onMouseDown={(e) => e.preventDefault()}
              ...
              {/* Add Field toggle */}
              <button onClick={() => setAddFieldMode((v) => !v)}
                title="Click then draw a rectangle on the PDF to add a custom field"
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition flex-shrink-0 ${addFieldMode ? 'bg-blue-500 text-white' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 hover:bg-blue-200'}`}>
              ...
              <button onClick={onClose} className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition flex-shrink-0">
                  ...
                  return (
                    <div key={pageIndex} className="mx-auto">
                      ...
                      <div
                        ...
                        style={{ width: Math.floor(page.width * displayScale), height: Math.floor(page.height * displayScale) }}
                      ...
                      >
                        ...
                        <div style={{ width: page.width, height: page.height, transform: `scale(${displayScale})`, transformOrigin: 'top left', position: 'absolute', top: 0, left: 0 }}>
                          <img src={page.dataUrl} alt={`Page ${pageIndex + 1}`}
                            style={{ width: page.width, height: page.height, display: 'block' }} draggable={false} />
                          {/* AcroForm inputs */}
                ...
                {photos.length > 0 && (
                  <div className="rounded-xl border border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/10 p-4">
                    <p className="text-xs font-semibold text-purple-700 dark:text-purple-400 mb-3 flex items-center gap-1.5">
                      <Camera size={12} /> {photos.length} photo{photos.length !== 1 ? 's' : ''} — each added as a PDF page
                    </p>
                    <div className="flex flex-wrap gap-3">
                      {photos.map((ph) => (
                        <div key={ph.id} className="relative group">
                          <img src={ph.dataUrl} alt={ph.name} className="w-28 h-28 object-cover rounded-xl border-2 border-purple-200 dark:border-purple-700 shadow-sm" />
                          <button onClick={() => setPhotos((p) => p.filter((x) => x.id !== ph.id))}
                            className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-red-500 hover:bg-red-600 text-white text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition shadow">✕</button>
                      ...
                      <button onClick={() => photoInputRef.current?.click()}
                        ...
                        <Plus size={22} /><span className="text-xs font-medium">Add more</span>
                      </button>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-gray-50 dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        value={sig1Name} onChange={(e) => setSig1Name(e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        type="date" value={sig1Date} onChange={(e) => setSig1Date(e.target.value)}
                        className="mt-1 w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-400"
                      />
                    </div>
                  </div>
                  ...
                  {!showSig2 ? (
                    <button onClick={() => setShowSig2(true)}
                    ...
                    <div className="border-t border-gray-200 dark:border-slate-700 pt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => { setShowSig2(false); setSig2DataUrl(null); setSig2Name(''); setSig2Date(TODAY); }} className="text-xs text-gray-400 hover:text-red-500 transition">Remove</button>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Client Name</label>
                          <input
                            value={sig2Name} onChange={(e) => setSig2Name(e.target.value)}
                            placeholder="Full name"
                            className="mt-1 w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-400"
                          />
                        </div>
                        <div>
                          <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Signature Date</label>
                          <input
                            type="date" value={sig2Date} onChange={(e) => setSig2Date(e.target.value)}
                            className="mt-1 w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-400"
                          />
                        </div>
                      </div>
          ...
          {saveMode && (
            <div className="border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-4 py-3 space-y-3 flex-shrink-0">
              <div className="space-y-1.5">
                ...
                {titleModified && (
                  <div className="flex gap-2 flex-wrap">
                    <button onClick={() => setFilename(documentTitle)}
                    ...
                    <button onClick={() => setFilename(originalFilename)}
              ...
              {saveMode === 'lead' && (
                <div className="space-y-2">
                  <div className="relative">
                    ...
                    <input value={leadSearch} onChange={(e) => { setLeadSearch(e.target.value); setShowLeadSearch(true); }} onFocus={() => setShowLeadSearch(true)}
                  ...
                  {selectedLead && (
                    <div className="flex items-center justify-between px-3 py-1.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                      ...
                      <button onClick={() => { setSelectedLead(null); setLeadSearch(''); }} className="text-amber-500 hover:text-red-500 transition"><X size={13} /></button>
                    </div>
                  )}
                  {showLeadSearch && filteredLeads.length > 0 && !selectedLead && (
                    <div className="max-h-36 overflow-y-auto rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg divide-y divide-gray-100 dark:divide-slate-800">
                      {filteredLeads.map((l) => (
                        <button key={l.id} onClick={() => { setSelectedLead(l); setLeadSearch(l.name); setShowLeadSearch(false); }}
                          ...
                          <span className="text-gray-400 ml-2 text-xs">{l.suburb} · {l.phone}</span>
                        </button>
                      ))}
                    </div>
                  )}
              ...
              <div className="flex gap-2">
                <button onClick={() => { setSaveMode(null); setSelectedLead(null); setLeadSearch(''); }}
                  className="px-4 py-1.5 text-sm rounded-lg border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700 transition">Cancel</button>
                <button onClick={saveMode === 'download' ? handleDownload : handleSaveToLead}
          ...
          {!saveMode && !loading && !loadError && (
            <div className="border-t border-gray-200 dark:border-slate-700 px-4 py-3 flex gap-2 flex-shrink-0">
              <button onClick={onClose}
                className="px-4 py-2 rounded-lg border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-gray-400 text-sm font-medium hover:bg-gray-100 dark:hover:bg-slate-700 transition">Cancel</button>
              <button onClick={() => openSaveMode('download')}
              ...
              <button onClick={() => openSaveMode('lead')}
      ...
      {fieldConfig && (
        <FieldConfigPopup
          ...
          onCancel={() => { setFieldConfig(null); setAddFieldMode(false); }}
        />
      )}
    </>
  );
