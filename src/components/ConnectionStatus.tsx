...
type DisplayState = "offline" | "syncing" | "synced" | "online";
...
export function ConnectionStatus({
  ...
  // Visibility: hide the indicator when everything is healthy and stable
  useEffect(() => {
  ...
  // "Synced ✓" flash: fires when the queue drains while online
  useEffect(() => {
    if (
      prevSyncing.current &&
      !isSyncing &&
    ...
    prevSyncing.current = isSyncing || queueLength > 0;
  ...
  return (
    ...
    >
      {displayState === "offline" && (
        <span className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-red-100 dark:bg-red-900/40 border border-red-200 dark:border-red-700/40 text-red-700 dark:text-red-300 text-[11px] font-semibold">
          <WifiOff size={11} aria-hidden="true" />
          {queueLength > 0
            ? `Offline · ${queueLength} queued`
            : "Offline"}
          {failedItems > 0 && (
            <span className="ml-1 inline-flex items-center justify-center w-4 h-4 rounded-full bg-amber-400 dark:bg-amber-500 text-white text-[9px] font-bold leading-none">
              {failedItems > 9 ? "9+" : failedItems}
            </span>
          )}
        </span>
      )}
      ...
      {displayState === "syncing" && (
        <span className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-amber-100 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700/40 text-amber-700 dark:text-amber-400 text-[11px] font-semibold">
...
export function SaveStateToast({ state, message }: SaveStateToastProps) {
  ...
  useEffect(() => {
    ...
    const dismiss = setTimeout(() => {
      setOpacity(false);
      // Give the CSS transition time to complete before unmounting
