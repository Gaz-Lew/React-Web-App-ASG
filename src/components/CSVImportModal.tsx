      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col">
              ...
              <div className="flex items-center gap-1 text-xs text-gray-400">
                {(['upload', 'map', 'preview'] as Step[]).map((s, i) => (
                  <React.Fragment key={s}>
                    ...
                    <span className={`capitalize font-medium ${step === s ? 'text-amber-500' : step === 'preview' && s === 'map' ? 'text-gray-500' : ''}`}>{s}</span>
                  </React.Fragment>
                ))}
              </div>
            </div>
            <button onClick={onClose} className="p-1 hover:bg-gray-100 dark:hover:bg-slate-800 rounded transition text-gray-500"><X size={18} /></button>
          </div>
          ...
          <div className="flex-1 overflow-auto px-6 py-5">
            ...
            {step === 'upload' && (
              ...
              >
                ...
                <p className="text-gray-600 dark:text-gray-400 font-medium mb-1">Drop CSV file here or click to browse</p>
            ...
            {step === 'map' && (
              <div className="space-y-4">
                <div className="flex items-center gap-4 flex-wrap">
                  <div>
                    <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Default Rep</label>
                    <select value={defaultRep} onChange={(e) => setDefaultRep(Number(e.target.value))}
                  ...
                  <div>
                    ...
                    <select value={defaultStatus} onChange={(e) => setDefaultStatus(e.target.value as LeadStatus)}
                      className="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white text-sm focus:outline-none">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 self-end ml-auto">{rows.length} rows detected</p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 dark:bg-slate-700">
                      <tr>
                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">CSV Column</th>
                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Maps To</th>
                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Sample</th>
                      </tr>
                    </thead>
                ...
                <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1">
                  <AlertTriangle size={12} /> Rows missing Name, Phone, or Suburb will be skipped ({previewLeads.length} of {rows.length} will be imported)
                </p>
              </div>
            )}
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-slate-600">
                      {previewLeads.map((l, i) => (
                        <tr key={i} className="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                          ...
                          <td className="px-3 py-2"><span className="px-1.5 py-0.5 rounded-full text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">{l.status}</span></td>
                          <td className="px-3 py-2 text-gray-500 dark:text-gray-400 text-xs">{reps.find((r) => r.id === l.dqRep)?.name ?? '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200 dark:border-slate-700 flex-shrink-0 bg-gray-50 dark:bg-slate-800">
            <button onClick={step === 'upload' ? onClose : () => setStep(step === 'preview' ? 'map' : 'upload')}
            ...
            {step === 'map' && (
              <button onClick={() => setStep('preview')} disabled={previewLeads.length === 0}
            ...
            {step === 'preview' && (
              <button onClick={handleImport} disabled={importing || previewLeads.length === 0}
                className="flex items-center gap-2 px-5 py-2 rounded-lg bg-amber-500 text-white font-semibold hover:bg-amber-400 disabled:opacity-50 transition text-sm">
                {importing ? 'Importing…' : <><Check size={15} /> Import {previewLeads.length} Leads</>}
              </button>
            )}
          </div>
        </div>
