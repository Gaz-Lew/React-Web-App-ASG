...
export function CallLogger({
  ...
  const buildAddress = (l: Lead): string =>
    [l.houseNum, l.street, l.suburb, l.postcode].filter(Boolean).join(' ');
  ...
  const validate = useCallback((): boolean => {
    ...
    if (
      ...
      newErrors.date =
        result === 'booked' ? 'Booking date required' : 'Callback date required';
    }
    ...
    if (
      ...
      newErrors.time =
        result === 'booked' ? 'Booking time required' : 'Callback time required';
    }
  ...
  const handleSave = useCallback(async () => {
    ...
    if (result === 'booked' && createAppt && apptServiceTypeId && bookingDate && bookingTime) {
      ...
      const repIdNum = apptRepId !== '' ? Number(apptRepId) : (reps.find(r => r.id === callingRep)?.id ?? reps[0]?.id ?? 0);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="px-6 py-4 space-y-4">
            ...
            {result === 'callback-today' && (
              <div className="px-3 py-2 bg-blue-50 dark:bg-blue-900/30 rounded-lg text-sm text-blue-800 dark:text-blue-200">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-2 py-1.5 text-sm rounded border border-amber-200 dark:border-amber-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-amber-500"
                      >
                        {reps.filter(r => r.active && r.availableForBookings !== false).map(r => (
              ...
              <div>
                ...
                <div className="space-y-2 max-h-[200px] overflow-y-auto">
                  {lead.callHistory.slice(-3).map((call, idx) => (
                    <div
                      key={idx}
                      className="p-2 bg-gray-50 dark:bg-slate-800 rounded text-xs"
                    >
