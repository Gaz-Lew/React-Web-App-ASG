...
export function AppointmentModal({
  ...
  const serviceTypesForRep = useMemo(() => {
    ...
    // If rep has no allowedServiceTypes configured, show ALL service types (permissive default)
    if (!rep?.allowedServiceTypes || rep.allowedServiceTypes.length === 0) return serviceTypes;
    const filtered = serviceTypes.filter((st) => rep.allowedServiceTypes!.includes(st.id));
    // If filtering results in empty list, fall back to all service types
    return filtered.length > 0 ? filtered : serviceTypes;
  }, [serviceTypes, reps, repId]);

  // Reset selected service type if it's not available for the selected rep
  useEffect(() => {
    if (serviceTypeId && serviceTypesForRep.length > 0) {
      const isValid = serviceTypesForRep.some((st) => st.id === serviceTypeId);
      if (!isValid) {
        setServiceTypeId("");
      }
    }
  }, [repId, serviceTypeId, serviceTypesForRep]);
  ...
  const handleSave = async () => {
    ...
    // Create centralized client note from appointment notes
    if (notes.trim() && linkedLeadId) {
      const serviceType = serviceTypes.find((s) => s.id === serviceTypeId);
      const apptType = serviceType?.name || "Appointment";
      try {
        await createClientNote({
          clientId: String(linkedLeadId),
          content: notes.trim(),
          source: "appointment",
          sourceId: id,
          appointmentType: apptType === "FC" ? "FC" : apptType === "FR" ? "FR" : "Discovery",
        ...
        // Non-fatal — appointment was saved successfully
        console.error("[AppointmentModal] Failed to create client note:", err);
      }
    }
  ...
  const inputClass =
    "w-full px-2.5 sm:px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-[var(--surface)] text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-amber-500";
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-[var(--surface)] rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg flex flex-col max-h-[95vh] sm:max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-gray-200 dark:border-white/[0.06] flex-shrink-0">
          <h2 className="text-sm sm:text-base font-semibold text-gray-800 dark:text-gray-200 truncate">
            {isEdit ? "Edit Appointment" : "New Appointment"}
          </h2>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-[var(--hover)] flex-shrink-0"
          >
            <X size={18} />
          </button>
        </div>
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2">
              {STATUS_OPTIONS.filter((o) => !o.isOutcome).map((opt) => (
                <button
                  ...
                  className={`px-2.5 sm:px-3 py-1.5 text-xs rounded-lg font-medium border transition-colors flex-shrink-0 ${
                    status === opt.value
        ...
        {/* Footer */}
        <div className="flex items-center gap-2 sm:gap-3 px-4 sm:px-5 py-3 sm:py-4 border-t border-gray-200 dark:border-white/[0.06] flex-shrink-0">
          {isEdit &&
            (confirmDelete ? (
              <div className="flex items-center gap-2">
                <span className="text-xs sm:text-sm text-red-600">Delete this appointment?</span>
                <button
                  onClick={() => onDelete(appointment!.id)}
                  className="px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm bg-red-500 text-white rounded-lg hover:bg-red-600 flex-shrink-0"
                >
                  Confirm
                </button>
                <button
                  onClick={() => setConfirmDelete(false)}
                  className="px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm text-gray-600 dark:text-gray-400 hover:underline flex-shrink-0"
                >
                  Cancel
                </button>
              </div>
          ...
          <button
            onClick={onClose}
            className="px-3 sm:px-4 py-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-[var(--hover)] rounded-lg flex-shrink-0"
          >
            Cancel
          </button>
          <button
