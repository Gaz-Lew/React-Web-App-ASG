...
interface AppointmentModalProps {
  ...
  prefilled: { date: string; time: string; repId?: number; clientName?: string; clientPhone?: string; linkedLeadId?: number } | null;
...
export function AppointmentModal({
  ...
  const [linkedLeadId, setLinkedLeadId] = useState<number | undefined>(appointment?.linkedLeadId ?? prefilled?.linkedLeadId);
  ...
  const bookableReps = useMemo(() =>
    reps.filter(r => r.active && r.availableForBookings !== false),
    [reps]
  );
  ...
  const PREFERRED_CATEGORY_ORDER = ['First Consult', 'Finance Run', 'Property Sale', 'SMSF', 'Coffee Runs', 'General', 'Finance'];
  ...
  // Lead search results
  const leadResults = leadSearch.length > 1
    ? leads.filter(l => {
  ...
  const handleSave = async () => {
    ...
    if (!serviceTypeId) { showToast('Service type is required', 'error'); return; }
    if (!repId) { showToast('Rep is required', 'error'); return; }
  ...
  const inputClass = 'w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-amber-500';
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              onFocus={() => { if (!clientName) setShowSuggestions(true); }}
            ...
            {showSuggestions && (
              ...
              >
                ...
                {suggestions.recentBooked.length > 0 && (
                  <>
                    ...
                    {suggestions.recentBooked.map(l => (
                      ...
                      >
                        <span className="text-sm font-medium text-gray-800 dark:text-gray-200 flex-1 truncate">{l.name}</span>
                ...
                {suggestions.recentFC.length > 0 && (
                  <>
                    <div className="px-3 pt-2 pb-1 text-[10px] font-bold text-indigo-600 uppercase tracking-wider bg-indigo-50/60 dark:bg-indigo-900/10 sticky top-0 z-10">
                    ...
                    {suggestions.recentFC.map(l => (
                      <button
                        key={l.id}
                        onMouseDown={() => pickSuggestion(l)}
                        className="w-full text-left px-3 py-2 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 flex items-center gap-2 border-b border-gray-50 dark:border-slate-800"
                      >
                        <span className="text-sm font-medium text-gray-800 dark:text-gray-200 flex-1 truncate">{l.name}</span>
                ...
                {suggestions.recentFR.length > 0 && (
                  <>
                    <div className="px-3 pt-2 pb-1 text-[10px] font-bold text-blue-600 uppercase tracking-wider bg-blue-50/60 dark:bg-blue-900/10 sticky top-0 z-10">
                      💰 Recent Finance Run
                    </div>
                    {suggestions.recentFR.map(l => (
                      <button
                        key={l.id}
                        onMouseDown={() => pickSuggestion(l)}
                        className="w-full text-left px-3 py-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 flex items-center gap-2 border-b border-gray-50 dark:border-slate-800"
                      >
                        <span className="text-sm font-medium text-gray-800 dark:text-gray-200 flex-1 truncate">{l.name}</span>
                ...
                {suggestions.recentBooked.length === 0 && suggestions.recentFC.length === 0 && suggestions.recentFR.length === 0 && (
          ...
          {/* Rep */}
          <div>
            <label className={labelClass}>Assigned Rep <span className="text-red-500">*</span></label>
            <select
              value={repId}
            ...
            >
              ...
              {bookableReps.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
            </select>
          </div>
          ...
          <div>
            ...
            {linkedLead ? (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700">
                ...
                <button
                  onClick={() => setLinkedLeadId(undefined)}
