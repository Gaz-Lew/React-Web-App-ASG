...
import {
  Phone, Users, Calendar, AlertTriangle, TrendingUp, Clock,
  Loader, Star, Activity, BarChart2, ArrowUp, ArrowRight,
  CheckCircle2, PhoneCall, UserCheck, Zap, ClipboardList,
  MapPin, FileText,
} from 'lucide-react';
...
function timeAgo(dateStr: string, timeStr?: string) {
  ...
  const dt = isNewFmt
    ...
    : new Date(timeStr || dateStr || ''); // old ISO fallback
...
// ── Stat Card ─────────────────────────────────────────────────────────────────
function StatCard({
  label, value, sub, icon, gradient, border, trend,
}: {
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-2xl p-4 sm:p-5 border ${border} shadow-sm hover:shadow-md dark:hover:shadow-slate-900/50 transition-all duration-200 group`}>
      <div className="flex items-start justify-between mb-3">
        <div className={`w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${gradient}`}>
          {icon}
        </div>
        {trend && (
          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex items-center gap-0.5 ${
            trend.dir === 'up' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400'
            : trend.dir === 'down' ? 'bg-red-50 dark:bg-red-900/30 text-red-500 dark:text-red-400'
            : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400'
          }`}>
            {trend.dir === 'up' ? <ArrowUp size={9} /> : trend.dir === 'down' ? <ArrowUp size={9} className="rotate-180" /> : null}
            {trend.label}
          </span>
        )}
      </div>
      <div className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white leading-none mb-1 tabular-nums">{value}</div>
...
// ── Calls Bar Chart ───────────────────────────────────────────────────────────
function CallsBarChart({ callsByDay }: { callsByDay: { label: string; shortDate: string; count: number; isToday: boolean }[] }) {
      ...
      {callsByDay.map((day) => {
        ...
        return (
          <div key={day.shortDate} className="flex-1 flex flex-col items-center gap-1 group/bar">
            <span className={`text-[10px] sm:text-xs font-semibold transition-opacity ${day.count > 0 ? 'opacity-100' : 'opacity-0 group-hover/bar:opacity-60'} ${day.isToday ? 'text-amber-500' : 'text-gray-500 dark:text-gray-400'}`}>
              {day.count || ''}
            </span>
            <div className="w-full flex items-end rounded-t-lg overflow-hidden" style={{ height: '80px' }}>
              <div
                className={`w-full rounded-t-lg transition-all duration-700 ease-out ${day.isToday ? 'bg-amber-500' : 'bg-indigo-400 dark:bg-indigo-500 group-hover/bar:bg-indigo-500 dark:group-hover/bar:bg-indigo-400'}`}
                style={{ height: `${Math.max(pct, day.count > 0 ? 6 : 2)}%`, opacity: day.count === 0 ? 0.25 : 1 }}
              />
            </div>
            <span className={`text-[10px] sm:text-xs font-medium ${day.isToday ? 'text-amber-500 font-bold' : 'text-gray-400 dark:text-gray-500'}`}>
              {day.label}
            </span>
          </div>
        );
...
// ── Conversion Funnel ─────────────────────────────────────────────────────────
function FunnelStep({ label, count, total, color, icon }: { label: string; count: number; total: number; color: string; icon: React.ReactNode }) {
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="w-full h-2 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all duration-700 ${color.replace('bg-', 'bg-').replace('/20', '')}`}
            style={{ width: `${pct}%`, backgroundColor: undefined }}
          />
        </div>
      </div>
      <span className="text-xs text-gray-400 dark:text-gray-500 w-10 text-right flex-shrink-0 tabular-nums">{pct}%</span>
    </div>
  );
}
...
function resultBadge(result?: string) {
  ...
  if (r.includes('book') || r.includes('appt')) return 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300';
  if (r.includes('interest')) return 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300';
  if (r.includes('callback') || r.includes('revisit')) return 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300';
  if (r.includes('not') || r.includes('wrong') || r.includes('no answer')) return 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400';
...
function QuickAction({
  label, icon, color, onClick, badge,
}: {
  label: string; icon: React.ReactNode; color: string; onClick?: () => void; badge?: number;
}) {
  return (
    <button
      onClick={onClick}
...
// ── Main Dashboard ────────────────────────────────────────────────────────────
export function DashboardPage({ onCallLead, onNavigate }: { onCallLead?: (lead: Lead) => void; onNavigate?: (page: string) => void }) {
  ...
  const stats = useMemo(() => {
    ...
    return { callsToday, callsThisWeek, booked, live, dq, newToday, newThisWeek, callbacks, overdueCount, total: leads.length, convRate };
  ...
  const statusCounts = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([label, value]) => ({ label, value, color: statusColors[label] ?? DEFAULT_STATUS_COLORS[label] ?? '#9ca3af' }))
  ...
  const recentActivity = useMemo(() => {
    return [...allCalls]
      .sort((a, b) => {
        ...
        const ta = isNewFmtB ? new Date(`${b.date}T${b.time || '00:00'}`).getTime() : new Date(b.time || b.date || '').getTime();
        const tb = isNewFmtA ? new Date(`${a.date}T${a.time || '00:00'}`).getTime() : new Date(a.time || a.date || '').getTime();
        return ta - tb;
      })
      .slice(0, 18);
  }, [allCalls]);
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-slate-950 p-3 sm:p-4 md:p-5 space-y-4 sm:space-y-5">
      ...
      >
        ...
        />
        {/* Gold accent line */}
        <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl" style={{ background: 'linear-gradient(to bottom, #c9a84c, #f59e0b, #c9a84c)' }} />
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 p-5 sm:p-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm mt-0.5" style={{ color: 'rgba(201,168,76,0.9)' }}>{fmtDate()}</p>
            <div className="flex flex-wrap items-center gap-2 mt-3">
              ...
              {stats.newToday > 0 && (
                <span className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold" style={{ background: 'rgba(201,168,76,0.2)', color: '#f5c842' }}>
                  <ArrowUp size={10} />
          ...
          <div className="hidden lg:flex flex-col items-end gap-0.5 flex-shrink-0 text-right">
            ...
            <span className="text-xs font-medium" style={{ color: 'rgba(201,168,76,0.8)' }}>calls today</span>
        ...
        <div className="relative border-t border-white/10 px-5 sm:px-6 py-3 flex items-center gap-2 overflow-x-auto scrollbar-none">
          ...
          <QuickAction
            ...
            color="bg-indigo-500/10 border-indigo-500/20 text-indigo-300 hover:bg-indigo-500/20"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
        <StatCard
          ...
          gradient="bg-indigo-50 dark:bg-indigo-900/30"
        ...
        />
        <StatCard
          label="Live Leads" value={stats.live}
        ...
        />
        <StatCard
          label="Callbacks Due" value={stats.callbacks.length}
          ...
          border={stats.overdueCount > 0 ? 'border-red-100 dark:border-red-900/50' : 'border-gray-100 dark:border-slate-700'}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        {/* 7-day call volume */}
        <div className="lg:col-span-3 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-4 sm:p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3 text-xs text-gray-400">
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded bg-indigo-400 dark:bg-indigo-500 opacity-80" />
                Previous
              </span>
              <span className="flex items-center gap-1.5">
        ...
        {/* Conversion funnel */}
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-4 sm:p-5">
          <div className="mb-4">
          ...
          <div className="space-y-3">
            <FunnelStep
              label="DQ'd" count={stats.dq} total={funnelTotal}
            ...
            <FunnelStep
              label="Live" count={stats.live} total={funnelTotal}
            ...
            <FunnelStep
              label="Booked" count={stats.booked} total={funnelTotal}
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-700">
            <div className="flex flex-wrap gap-1.5">
              {statusCounts.slice(0, 6).map(s => (
                <span key={s.label} className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-gray-50 dark:bg-slate-700 text-gray-600 dark:text-gray-400">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto pr-0.5">
              {stats.callbacks.slice(0, 8).map((lead) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${overdue ? 'bg-red-500 text-white' : 'bg-amber-500 text-white'}`}>
                      {repInitial(rep)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="font-semibold text-gray-900 dark:text-white text-xs truncate">{lead.name}</span>
                        {overdue && <span className="text-[9px] font-bold text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/50 px-1 py-0.5 rounded flex-shrink-0">LATE</span>}
                      </div>
                    ...
                    {onCallLead && (
                      <button onClick={() => onCallLead(lead)} className="flex-shrink-0 p-1.5 rounded-lg bg-amber-500 text-white hover:bg-amber-400 transition">
                        <Phone size={12} />
                      </button>
                    )}
                  </div>
            ...
            <div className="space-y-0 max-h-64 overflow-y-auto divide-y divide-gray-50 dark:divide-slate-700/50">
              {recentActivity.map((call, i) => (
                <div key={i} className="flex items-center gap-3 py-2.5 first:pt-0">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-400 to-indigo-600 flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0">
                    {repInitial(call.rep)}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  {call.result && (
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold flex-shrink-0 ${resultBadge(call.result)}`}>
                      {call.result}
                    </span>
                  )}
                </div>
        ...
        {repStats.length === 0 ? (
          ...
          <>
            ...
            <div className="hidden sm:grid grid-cols-[2rem_1fr_3fr_5rem_5rem_5rem] gap-3 items-center mb-2 px-1">
              <span />
              <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Rep</span>
              <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Activity</span>
              <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider text-right">Calls</span>
              <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider text-right">This Week</span>
              <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider text-right">Booked</span>
            </div>
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 ${isTop3 ? 'bg-amber-500' : 'bg-slate-400 dark:bg-slate-600'}`}>
                          {repInitial(item.rep.name)}
                        </div>
                        <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{item.rep.name}</span>
                        {item.callsToday > 0 && (
                    ...
                    <div className="hidden sm:flex items-center gap-2">
                      <div className="flex-1 h-2 bg-gray-200 dark:bg-slate-700 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-700 ${isTop3 ? 'bg-amber-500' : 'bg-indigo-400 dark:bg-indigo-500'}`}
                          style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                    ...
                    {/* This week */}
                    <div className="hidden sm:block text-right">
                      <span className={`text-sm font-bold tabular-nums ${item.callsWeek > 0 ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 dark:text-gray-500'}`}>
                        {item.callsWeek}
                      </span>
                    </div>
                    ...
                    {/* Booked */}
                    <div className="hidden sm:block text-right">
                      <span className={`text-sm font-bold tabular-nums ${item.bookedCount > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400 dark:text-gray-500'}`}>
                        {item.bookedCount}
                      </span>
                    </div>
                  </div>
