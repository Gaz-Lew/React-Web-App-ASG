...
import { LibraryDocument, DocFormField, Lead, FormTemplate, FormTemplateField, FormFieldType, FormFieldAutoFill } from '../types';
import { useAppStore } from '../stores/appStore';
import {
  useDocumentLibrary, useSaveLibraryDocument, useDeleteLibraryDocument, useLeads,
...
import {
  FolderOpen, Upload, Download, Search, Trash2, X, Plus, ChevronDown,
  FileText, Link2, Loader, ClipboardList, Edit2, GripVertical, ArrowUpDown, Check,
} from 'lucide-react';
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const libDoc: LibraryDocument = {
        id, name: name.trim(), description: description.trim(), category,
        storagePath: path, downloadUrl: url,
        fileType: file.type || 'application/octet-stream', fileSize: file.size,
        uploadedBy: currentUser?.name ?? 'Admin', uploadedAt: now,
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div>
            ...
            <input className={inp} value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Client Consent Form" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Description</label>
            <textarea className={inp} rows={2} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Brief description" />
          </div>
          <div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={addField} className="text-xs text-amber-600 dark:text-amber-400 hover:underline flex items-center gap-1">
                <Plus size={11} /> Add Field
              </button>
            </div>
            {formFields.map((field) => (
              <div key={field.id} className="flex items-center gap-2 p-2 mb-2 rounded-lg bg-gray-50 dark:bg-slate-800">
                <input className="flex-1 px-2 py-1 text-xs rounded border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:outline-none"
                  placeholder="Field label" value={field.label} onChange={(e) => updateField(field.id, { label: e.target.value })} />
                <select className="px-2 py-1 text-xs rounded border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:outline-none"
                  value={field.autoFill ?? ''} onChange={(e) => updateField(field.id, { autoFill: e.target.value as DocFormField['autoFill'] || undefined })}>
                  <option value="">Manual entry</option>
                  {Object.entries(AUTO_FILL_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                </select>
                <button onClick={() => removeField(field.id)} className="text-red-400 hover:text-red-500 p-0.5"><X size={12} /></button>
              </div>
            ))}
          </div>
        </div>
        <div className="px-5 py-4 border-t border-gray-200 dark:border-slate-700 flex gap-3">
          ...
          <button onClick={onClose} className="px-4 py-2.5 rounded-xl border border-gray-300 dark:border-slate-600 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-800 transition">
            Cancel
          </button>
        </div>
      </div>
...
function UseForLeadModal({ document: libDoc, onClose }: { document: LibraryDocument; onClose: () => void }) {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter((l) =>
      l.name.toLowerCase().includes(term) || l.phone?.includes(term) || l.suburb?.toLowerCase().includes(term)
    ).slice(0, 10);
  }, [leads, leadSearch]);
  ...
  const getAutoFill = (key: DocFormField['autoFill'], lead: Lead): string => {
    ...
    if (key === 'leadAddress') return [lead.houseNum, lead.street, lead.suburb, lead.postcode].filter(Boolean).join(' ');
  ...
  const handleSelectLead = (lead: Lead) => {
    ...
    (libDoc.formFields ?? []).forEach((f) => { if (f.autoFill) auto[f.id] = getAutoFill(f.autoFill, lead); });
  ...
  const buildPDF = (): { blob: Blob; filename: string } => {
    ...
    let headerY = 32;
    if (selectedLead) { pdf.text(`Client: ${selectedLead.name}`, 15, headerY); headerY += 7; }
  ...
  const handleSaveToLead = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'leads', String(selectedLead.id), 'files'), {
        ...
        fileType: 'application/pdf', fileSize: blob.size, type: 'document',
        uploadedBy: currentUser?.name ?? '', uploadedAt: Date.now(), linkedDocumentId: libDoc.id,
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-slate-700">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-400 flex-shrink-0"><X size={16} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          <div>
            ...
            {(libDoc.formFields ?? []).length > 0 ? (
              ...
              <p className="text-xs text-gray-400 bg-gray-50 dark:bg-slate-800 rounded-lg px-3 py-2">
                This document has no form fields defined. A cover sheet with the document name and date will be generated.
              </p>
            )}
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">
              ...
              <span className="ml-1.5 text-gray-400 dark:text-gray-500 font-normal normal-case">(optional — skip to download only)</span>
            </label>
              ...
              <div className="mt-1 border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden bg-white dark:bg-slate-800 shadow-sm">
                {filteredLeads.length === 0 ? (
                  ...
                  >
                    ...
                    <span className="text-xs text-gray-400">{lead.phone ? `${lead.phone} · ` : ''}{lead.suburb}</span>
                  </button>
                ))}
              </div>
            )}
            {selectedLead && (
              <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
                ...
                <button onClick={() => setSelectedLead(null)} className="ml-auto text-amber-400 hover:text-amber-600"><X size={12} /></button>
              </div>
            )}
          </div>
        </div>
...
function DocumentCard({ doc: libDoc, isAdmin, onDelete }: { doc: LibraryDocument; isAdmin: boolean; onDelete: () => void }) {
    ...
    <>
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-4 flex flex-col gap-3 hover:shadow-md transition group">
        ...
        {libDoc.fileType?.includes('pdf') && libDoc.downloadUrl && (
          <PdfThumbnail url={libDoc.downloadUrl} />
        )}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {libDoc.description && <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-2">{libDoc.description}</p>}
          </div>
          {isAdmin && (
            <div className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition">
              {!confirmDelete ? (
                <button onClick={() => setConfirmDelete(true)} className="p-1 rounded text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition">
                  <Trash2 size={13} />
                </button>
              ) : (
                <div className="flex items-center gap-1">
                  <button onClick={onDelete} className="px-2 py-0.5 rounded text-xs bg-red-500 text-white font-medium hover:bg-red-400">Del</button>
                  <button onClick={() => setConfirmDelete(false)} className="px-2 py-0.5 rounded text-xs border border-gray-300 dark:border-slate-600 text-gray-500 hover:bg-gray-50 dark:hover:bg-slate-800">✕</button>
                </div>
              )}
            </div>
          )}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="px-2 py-0.5 rounded-full text-xs bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-400">{libDoc.category}</span>
        ...
        <div className="flex gap-2 mt-auto pt-1">
          <a href={libDoc.downloadUrl} target="_blank" rel="noopener noreferrer" download={libDoc.name}
          ...
          <button onClick={() => setUseForLead(true)}
...
function FormBuilderModal({
  ...
  onClose,
  onSaved,
}: { template?: FormTemplate; onClose: () => void; onSaved: () => void }) {
  const { currentUser } = useAppStore();
  ...
  const handleSave = async () => {
    ...
    setSaving(false);
    if (ok) { showToast(`✅ Form template "${name}" saved`, 'success'); onSaved(); onClose(); }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Description</label>
            <textarea className={inp} rows={2} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="What this form is used for" />
          </div>
          ...
          <div>
            ...
            {fields.length === 0 && (
              <p className="text-xs text-gray-400 italic">No fields yet. Add at least one field and a Signature field.</p>
            )}

            <div className="space-y-2">
              {fields.map((field, idx) => (
                <div key={field.id} className="p-3 rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    >
                      {Object.entries(FORM_FIELD_TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                    </select>
                    <button onClick={() => removeField(field.id)} className="text-red-400 hover:text-red-500 p-0.5 flex-shrink-0">
                      <X size={13} />
                    </button>
                  </div>
                    ...
                    {AUTO_FILL_SUPPORTED.includes(field.type) && (
                      <div className="flex items-center gap-1.5">
                        ...
                        <select
                          ...
                          onChange={(e) => updateField(field.id, { autoFill: e.target.value as FormFieldAutoFill || undefined })}
                        >
            ...
            {!fields.some((f) => f.type === 'signature') && (
              <button
                onClick={() => setFields((prev) => [...prev, { id: `sig_${Date.now()}`, label: 'Signature', type: 'signature' }])}
...
function PdfTemplateUploadModal({
  ...
  const handleSave = async () => {
    ...
    if (!existing && !file) { showToast('Please select a PDF file', 'error'); return; }
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md">
          ...
          <div className="px-6 py-5 space-y-4">
            <div>
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Client Agreement" className={inp} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Description</label>
              <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Optional" className={inp} />
            </div>
            <div>
          ...
          <div className="px-6 py-4 border-t border-gray-200 dark:border-slate-700 flex gap-3">
            <button onClick={onClose} className="flex-1 py-2 rounded-lg border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-300 text-sm font-medium hover:bg-gray-50 dark:hover:bg-slate-800 transition">
              Cancel
            </button>
            <button
              onClick={handleSave}
              disabled={uploading}
              className="flex-1 py-2 rounded-lg bg-blue-500 text-white text-sm font-semibold hover:bg-blue-400 transition disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {uploading ? <><Loader size={14} className="animate-spin" /> Uploading…</> : (existing ? 'Save Changes' : 'Upload Template')}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-4 flex flex-col gap-3 hover:shadow-md transition group">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 flex-shrink-0">PDF</span>
        ...
        {isAdmin && (
          <div className="flex-shrink-0 flex gap-1 opacity-0 group-hover:opacity-100 transition">
            ...
            {confirmDelete ? (
              <div className="flex items-center gap-1">
                <button onClick={onDelete} className="px-2 py-0.5 rounded text-xs bg-red-500 text-white font-medium">Del</button>
                <button onClick={onCancelDelete} className="px-2 py-0.5 rounded text-xs border border-gray-300 dark:border-slate-600 text-gray-500">✕</button>
              </div>
            ) : (
              <button onClick={onConfirmDelete} className="p-1 rounded text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition">
                <Trash2 size={13} />
              </button>
            )}
          </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400">
        ...
        {isAdmin && (
          <>
            <button
              ...
              disabled={replacing}
              className="w-full flex items-center justify-center gap-2 py-1.5 rounded-lg border border-blue-300 dark:border-blue-700 text-blue-600 dark:text-blue-400 text-xs font-medium hover:bg-blue-50 dark:hover:bg-blue-900/20 transition disabled:opacity-50"
...
function FormsTab({ isAdmin }: { isAdmin: boolean }) {
  ...
  const handleDelete = async (t: FormTemplate) => {
    ...
    if (t.type === 'pdf' && t.pdfStoragePath) {
      try { await deleteFile(t.pdfStoragePath); } catch { /* ignore — file may already be gone */ }
    }
  ...
  const startReorder = () => { setReorderList([...templates]); setIsReordering(true); };
  ...
  const handleFormDragStart = (i: number) => { dragIdxRef.current = i; };
    ...
    <div className="space-y-4" onClick={() => showNewMenu && setShowNewMenu(false)}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        {isAdmin && !isReordering && (
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button
                onClick={(e) => { e.stopPropagation(); setShowNewMenu((prev) => !prev); }}
              ...
              {showNewMenu && (
                <div className="absolute right-0 top-full mt-1 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 shadow-xl z-20 overflow-hidden min-w-[200px]">
                  <button
                    onClick={() => { setShowNewMenu(false); setEditingTemplate(undefined); setBuilderOpen(true); }}
                  ...
                  <button
                    onClick={() => { setShowNewMenu(false); setEditingTemplate(undefined); setPdfUploadOpen(true); }}
                    className="w-full text-left px-4 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition flex items-center gap-2 border-t border-gray-100 dark:border-slate-800"
                  >
                    <Upload size={14} className="text-blue-500" /> Upload PDF Form
                  </button>
                </div>
              )}
            </div>
        ...
        {isAdmin && isReordering && (
          <div className="flex items-center gap-2">
            ...
            <button onClick={saveFormOrder} disabled={savingOrder}
      ...
      {isReordering ? (
        <div className="space-y-1.5">
          {reorderList.map((t, i) => (
            ...
            >
              <GripVertical size={16} className="text-gray-300 dark:text-gray-600 flex-shrink-0 group-hover:text-amber-400 transition" />
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-gray-400">
            {isAdmin ? 'Click "New Form Template" to create your first fillable form.' : 'Ask your admin to create form templates.'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {templates.map((t) => {
            ...
            if (isPdf) {
              return (
                <PdfTemplateCard
                  ...
                  onEdit={() => { setEditingTemplate(t); setPdfUploadOpen(true); }}
            ...
            return (
              <div key={t.id} className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-4 flex flex-col gap-3 hover:shadow-md transition group">
                {/* Header */}
                <div className="flex items-start gap-3">
                  <span className="text-2xl flex-shrink-0">📋</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <h3 className="text-sm font-semibold text-gray-900 dark:text-white truncate">{t.name}</h3>
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 flex-shrink-0">BUILDER</span>
                    </div>
                    {t.description && <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-2">{t.description}</p>}
                  </div>
                  {isAdmin && (
                    <div className="flex-shrink-0 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                      ...
                      {confirmDelete === t.id ? (
                        <div className="flex items-center gap-1">
                          <button onClick={() => handleDelete(t)} className="px-2 py-0.5 rounded text-xs bg-red-500 text-white font-medium">Del</button>
                          <button onClick={() => setConfirmDelete(null)} className="px-2 py-0.5 rounded text-xs border border-gray-300 dark:border-slate-600 text-gray-500">✕</button>
                        </div>
                      ) : (
                        <button onClick={() => setConfirmDelete(t.id)} className="p-1 rounded text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition">
                          <Trash2 size={13} />
                        </button>
                      )}
                    </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {t.fields.slice(0, 4).map((f) => (
                    <span key={f.id} className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      f.type === 'signature'
      ...
      {builderOpen && (
        <FormBuilderModal
          ...
          onClose={() => { setBuilderOpen(false); setEditingTemplate(undefined); }}
          onSaved={() => { setBuilderOpen(false); setEditingTemplate(undefined); }}
        />
      )}
      ...
      {pdfUploadOpen && (
        <PdfTemplateUploadModal
          ...
          onClose={() => { setPdfUploadOpen(false); setEditingTemplate(undefined); }}
          onSaved={() => { setPdfUploadOpen(false); setEditingTemplate(undefined); }}
        />
      )}
...
export function DocumentCentrePage() {
  ...
  const saveDocOrder = async () => {
    setSavingOrder(true);
    await Promise.all(
      reorderDocs.map((d, i) => saveDoc({ ...d, sortOrder: i }))
    );
    setSavingOrder(false);
  ...
  const handleDocDragStart = (i: number) => { dragIndexRef.current = i; };
    ...
    <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-slate-950 p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {isAdmin && activeTab === 'documents' && !isReorderingDocs && (
          <div className="flex items-center gap-2">
            {documents.length > 1 && (
              <button onClick={startReorderDocs}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-600 dark:text-gray-400 text-sm font-medium hover:border-amber-400 hover:text-amber-600 transition">
                <ArrowUpDown size={13} /> Reorder
              </button>
            )}
        ...
        {isAdmin && activeTab === 'documents' && isReorderingDocs && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-400 dark:text-gray-500">Drag rows to reorder</span>
            <button onClick={() => setIsReorderingDocs(false)}
              className="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-slate-700 text-sm text-gray-500 hover:bg-gray-100 dark:hover:bg-slate-800 transition">
              Cancel
            </button>
            <button onClick={saveDocOrder} disabled={savingOrder}
              className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-green-500 text-white text-sm font-semibold hover:bg-green-400 disabled:opacity-50 transition">
              <Check size={13} /> {savingOrder ? 'Saving…' : 'Save Order'}
            </button>
          </div>
      ...
      <div className="flex gap-1 border-b border-gray-200 dark:border-slate-700">
        {([['documents', '📄 Documents'], ['forms', '📋 Forms']] as const).map(([key, label]) => (
          <button key={key} onClick={() => { setActiveTab(key); setIsReorderingDocs(false); }}
              ...
              {reorderDocs.map((doc, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 truncate">{doc.category}{doc.description ? ` · ${doc.description}` : ''}</p>
                  </div>
                  <span className="text-xs text-gray-300 dark:text-gray-600 flex-shrink-0 tabular-nums">#{i + 1}</span>
                </div>
              ))}
            ...
            <>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="relative flex-1 min-w-48 max-w-sm">
                  ...
                  <input className="w-full pl-9 pr-3 py-2 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-400"
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-sm text-gray-400">
                    {isAdmin ? 'Click "Upload Document" to add the first template.' : 'Ask your admin to upload documents.'}
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
