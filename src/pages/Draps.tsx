...
import {
  BarChart3, Plus, Trash2, Calendar, ChevronDown, ChevronUp,
  Download, Trophy, TrendingUp,
} from 'lucide-react';
...
const emptyForm = () => ({
  ...
  fcAppts: 0, fcPresented: 0, fcBooked: 0,
  frAppts: 0, frPresented: 0, frBooked: 0,
});
    ...
    <div className="flex flex-col items-center gap-1 flex-1 min-w-0">
      ...
      {pct !== undefined && (
        <div className={`text-xs font-semibold px-1.5 py-0.5 rounded-full ${
          pct >= 50 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
          : pct >= 25 ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'
...
function FunnelArrow() {
  return (
    <div className="flex-shrink-0 text-gray-300 dark:text-slate-600 text-lg self-center pb-5">›</div>
  );
}
...
function MiniBar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5 min-w-0">
      ...
      <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 tabular-nums w-6 text-right">{value}</span>
    </div>
  );
}

// ── Target progress bar ───────────────────────────────────────────────────────
function TargetBar({ label, value, target, color = 'bg-amber-500' }: {
...
function exportCSV(entries: DrapsEntry[], filename: string) {
  const headers = ['Date', 'Rep', 'DQ', 'Referrals', 'Appointments', 'Presentations', 'Sold',
    'FC Appts', 'FC Presented', 'FC Booked', 'FR Appts', 'FR Presented', 'FR Booked'];
  const rows = entries.map((e) => [
    e.date, e.repName, e.dq, e.referrals, e.appointments, e.presentations, e.sold,
    e.fcAppts, e.fcPresented, e.fcBooked, e.frAppts, e.frPresented, e.frBooked,
  ].join(','));
...
export function DrapsPage() {
  ...
  const setF = (field: keyof typeof form, val: string | number) =>
    setForm((p) => ({ ...p, [field]: val }));

  const handleSave = async () => {
    if (!form.repId) { showToast('Please select a rep', 'error'); return; }
  ...
  const totals = useMemo(() => filteredEntries.reduce(
    ...
    { dq: 0, referrals: 0, appointments: 0, presentations: 0, sold: 0,
      fcAppts: 0, fcPresented: 0, fcBooked: 0, frAppts: 0, frPresented: 0, frBooked: 0 }
  ), [filteredEntries]);

  // ── Conversion rates ───────────────────────────────────────────────────────
  const pct = (num: number, den: number) =>
    den > 0 ? Math.round((num / den) * 100) : undefined;

  // ── Per-rep leaderboard ────────────────────────────────────────────────────
  const leaderboard = useMemo(() => {
    const map = new Map<number, {
      repId: number; repName: string;
      dq: number; referrals: number; appointments: number; presentations: number; sold: number;
      fcBooked: number; frBooked: number; entries: number;
    }>();
    filteredEntries.forEach((e) => {
      ...
      if (existing) {
        ...
        map.set(e.repId, {
          ...
          dq: e.dq, referrals: e.referrals, appointments: e.appointments,
          presentations: e.presentations, sold: e.sold,
          fcBooked: e.fcBooked, frBooked: e.frBooked, entries: 1,
        });
      }
    });
  ...
  const groupedHistory = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([date, rows]) => ({
      date,
      rows,
      totals: rows.reduce((acc, e) => ({
        dq: acc.dq + e.dq, referrals: acc.referrals + e.referrals,
        appointments: acc.appointments + e.appointments,
        presentations: acc.presentations + e.presentations,
        sold: acc.sold + e.sold,
      }), { dq: 0, referrals: 0, appointments: 0, presentations: 0, sold: 0 }),
  ...
  const toggleLbSort = (col: LeaderboardSort) => {
    ...
    else { setLbSort(col); setLbDir('desc'); }
  };
  ...
  const rangeLabels: Record<DateRange, string> = {
    week: 'Last 7 Days', month: 'This Month', quarter: 'This Quarter', all: 'All Time', custom: 'Custom Range',
  };
  ...
  const rankBg = ['bg-amber-50 dark:bg-amber-900/10', 'bg-gray-50 dark:bg-slate-800/30', 'bg-orange-50 dark:bg-orange-900/10'];
    ...
    <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-slate-950 p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-400 dark:text-gray-500 hidden sm:block">Daily Reporting & Activity Performance Stats</span>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
        ...
        <div className="mb-3">
          <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">Field Metrics</p>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
        ...
        <div className="flex items-start gap-1 sm:gap-2">
          ...
          <FunnelArrow />
          <FunnelStep label="Referrals" value={totals.referrals} pct={pct(totals.referrals, totals.dq)} color="text-purple-600 dark:text-purple-400" />
          <FunnelArrow />
          <FunnelStep label="Appointments" value={totals.appointments} pct={pct(totals.appointments, totals.dq)} color="text-amber-600 dark:text-amber-400" />
          <FunnelArrow />
          <FunnelStep label="Presentations" value={totals.presentations} pct={pct(totals.presentations, totals.appointments)} color="text-orange-600 dark:text-orange-400" />
          <FunnelArrow />
          <FunnelStep label="Sold" value={totals.sold} pct={pct(totals.sold, totals.presentations)} color="text-green-600 dark:text-green-400" />
        </div>
        ...
        {totals.dq > 0 && (
          <div className="mt-4 pt-3 border-t border-gray-100 dark:border-slate-800 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span className={`font-bold text-sm ${
              (totals.sold / totals.dq) >= 0.1 ? 'text-green-600 dark:text-green-400'
              : (totals.sold / totals.dq) >= 0.05 ? 'text-amber-600 dark:text-amber-400'
              : 'text-red-500 dark:text-red-400'
            }`}>{pct(totals.sold, totals.dq)}%</span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
        {[
          ...
          <div key={label} className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-4 text-center">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        {[
          ...
          { label: 'FC Presented', value: totals.fcPresented, color: 'text-indigo-600 dark:text-indigo-400', sub: 'FC' },
          ...
          <div key={label} className={`bg-white dark:bg-slate-900 rounded-xl border p-3 text-center ${
              ...
              <thead className="bg-gray-50 dark:bg-slate-800">
                <tr>
                  ...
                  {([
                    ...
                    ['fcBooked', 'FC Booked'],
                    ['frBooked', 'FR Booked'],
                  ] as [LeaderboardSort, string][]).map(([col, label]) => (
                    <th
                      key={col}
                  ...
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">vs Target</th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      <td className="px-3 py-2">
                        <span className={`text-sm font-bold ${isTop3 ? rankColors[idx] : 'text-gray-400 dark:text-gray-500'}`}>
                          {idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : idx + 1}
                        </span>
                      </td>
                      <td className="px-3 py-2">
                        <div className="font-medium text-gray-900 dark:text-white whitespace-nowrap">{rep.repName}</div>
                        <div className="text-xs text-gray-400">{rep.entries} {rep.entries === 1 ? 'day' : 'days'}</div>
                      </td>
                      <td className="px-3 py-2 min-w-[100px]">
                        <MiniBar value={rep.dq} max={lbMax.dq} color="bg-blue-500" />
                      </td>
                      ...
                      <td className="px-3 py-2 min-w-[80px]">
                        <MiniBar value={rep.fcBooked} max={lbMax.fcBooked} color="bg-indigo-500" />
                      </td>
                      <td className="px-3 py-2 min-w-[80px]">
            ...
            {groupedHistory.map(({ date, rows, totals: gt }) => {
              ...
              return (
                <div key={date} className="border-b border-gray-100 dark:border-slate-800 last:border-b-0">
                  ...
                  >
                    {collapsed ? <ChevronDown size={13} className="text-gray-400 flex-shrink-0" /> : <ChevronUp size={13} className="text-gray-400 flex-shrink-0" />}
                    ...
                    <div className="hidden sm:flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                      ...
                      {gt.appointments > 0 && <span className="text-amber-600 dark:text-amber-400 font-medium">{gt.appointments} Appts</span>}
                      {gt.sold > 0 && <span className="text-green-600 dark:text-green-400 font-medium">{gt.sold} Sold</span>}
                    </div>
                  </button>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800">
                          <tr>
                            {['Rep', 'DQ', 'Refs', 'Appts', 'Pres', 'Sold', 'FC Appts', 'FC Pres', 'FC Booked', 'FR Appts', 'FR Pres', 'FR Booked', ''].map((h) => (
                              <th key={h} className="px-3 py-1.5 text-left text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide whitespace-nowrap">
                                {h}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody className="divide-y divide-gray-50 dark:divide-slate-800/50">
                          {rows.map((e) => (
                            <tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/40 transition">
                              <td className="px-3 py-2 whitespace-nowrap font-medium text-gray-900 dark:text-white">{e.repName}</td>
                              <td className="px-3 py-2 text-center font-semibold text-blue-600 dark:text-blue-400">{e.dq}</td>
                              <td className="px-3 py-2 text-center text-gray-600 dark:text-gray-400">{e.referrals}</td>
                              <td className="px-3 py-2 text-center text-gray-600 dark:text-gray-400">{e.appointments}</td>
                              <td className="px-3 py-2 text-center text-gray-600 dark:text-gray-400">{e.presentations}</td>
                              <td className="px-3 py-2 text-center font-semibold text-green-600 dark:text-green-400">{e.sold}</td>
                        ...
                        {rows.length > 1 && (
                          <tfoot>
                            <tr className="bg-gray-50 dark:bg-slate-800/60 font-semibold text-xs">
                              <td className="px-3 py-1.5 text-gray-500 dark:text-gray-400 uppercase tracking-wide">Day Total</td>
                              <td className="px-3 py-1.5 text-center text-blue-600 dark:text-blue-400">{gt.dq}</td>
                              <td className="px-3 py-1.5 text-center text-gray-500 dark:text-gray-400">{gt.referrals}</td>
                              <td className="px-3 py-1.5 text-center text-gray-500 dark:text-gray-400">{gt.appointments}</td>
                              <td className="px-3 py-1.5 text-center text-gray-500 dark:text-gray-400">{gt.presentations}</td>
