...
import { useLeads, useSaveLead, useDeleteLead, useKnockZones, useSaveKnockZone, useDeleteKnockZone, useCustomPinTypes, useSaveCustomPinType, useDeleteCustomPinType } from '../hooks/useFirebase';
import { useToast } from '../context/ToastContext';
import { MapPin, X, Navigation, Crosshair, Layers, Trash2, PenLine, Undo2, Plus, Maximize2, Minimize2, Eye, EyeOff, Search, Thermometer } from 'lucide-react';
...
const LOCATION_CACHE_KEY = 'asgUserLocation';
...
const ALL_STATUSES: LeadStatus[] = [
  'DQ', 'Booked', 'Revisit', 'Not Interested', 'Wrong Number', 'No Answer',
];
...
// ── Knock modal ───────────────────────────────────────────────────────────────
interface KnockForm {
  name: string; phone: string; houseNum: string; street: string;
  suburb: string; postcode: string; notes: string;
  knockResult: KnockResult;
}

const KNOCK_RESULT_KEYS: KnockResult[] = [
  'no-answer', 'not-interested', 'skipped', 'dq-complete', 'parents-not-home',
];

function KnockModal({
  latLng, geocodedAddress, onClose, onSave,
}: {
  ...
  const [form, setForm] = useState<KnockForm>({
    name: '', phone: '', knockResult: 'no-answer',
  ...
  const inp = 'w-full px-2 py-1.5 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-400';
      ...
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-slate-700">
            ...
            <button onClick={onClose} className="p-1 hover:bg-gray-100 dark:hover:bg-slate-800 rounded transition"><X size={16} /></button>
          </div>
          <div className="px-5 py-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Name</label><input className={inp} value={form.name} onChange={(e) => sf('name', e.target.value)} placeholder="Resident name" /></div>
              <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Phone</label><input className={inp} value={form.phone} onChange={(e) => sf('phone', e.target.value)} placeholder="04xx xxx xxx" /></div>
              <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">House # *</label><input className={inp} value={form.houseNum} onChange={(e) => sf('houseNum', e.target.value)} placeholder="12" /></div>
              <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Street *</label><input className={inp} value={form.street} onChange={(e) => sf('street', e.target.value)} placeholder="Main St" /></div>
              <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Suburb *</label><input className={inp} value={form.suburb} onChange={(e) => sf('suburb', e.target.value)} placeholder="Suburb" /></div>
              <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Postcode *</label><input className={inp} value={form.postcode} onChange={(e) => sf('postcode', e.target.value)} placeholder="3000" /></div>
            </div>
            {/* Knock result — colour-coded chips */}
            <div>
              <label className="text-xs text-amber-600 dark:text-amber-400 font-semibold mb-2 block tracking-wide">Knock Result</label>
              <div className="grid grid-cols-1 gap-1.5">
                {KNOCK_RESULT_KEYS.map((k) => (
                  <button
                    ...
                    style={form.knockResult === k
                      ? { backgroundColor: KNOCK_COLORS[k] + '22', borderColor: KNOCK_COLORS[k], color: KNOCK_COLORS[k] }
            ...
            <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Notes</label><textarea className={inp + ' resize-none'} rows={2} value={form.notes} onChange={(e) => sf('notes', e.target.value)} /></div>
          </div>
          <div className="flex gap-3 px-5 py-4 border-t border-gray-200 dark:border-slate-700">
            <button onClick={onClose} className="flex-1 py-2 rounded-lg border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-300 text-sm hover:bg-gray-50 dark:hover:bg-slate-800 transition">Cancel</button>
            <button onClick={handleSave} disabled={!form.houseNum || !form.street || !form.suburb || !form.postcode}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-sm">
          ...
          <div className="px-5 py-4 space-y-3">
            ...
            <div>
              <label className="text-xs text-amber-600 dark:text-amber-400 font-semibold mb-1 block">Zone Name <span className="font-normal text-gray-400">(optional)</span></label>
              <input className={inp} value={zoneName} onChange={(e) => setZoneName(e.target.value)} placeholder="e.g. Block A North" />
            </div>
                ...
                {reps.map((r) => (
                  ...
                  >
                    <span className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition ${
                      selectedRepIds.has(r.id) ? 'bg-amber-500 border-amber-500' : 'border-gray-300 dark:border-slate-600'
                    }`}>
          ...
          <div className="flex gap-3 px-5 py-4 border-t border-gray-200 dark:border-slate-700">
            <button onClick={onClose} className="flex-1 py-2 rounded-lg border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-300 text-sm hover:bg-gray-50 dark:hover:bg-slate-800 transition">Cancel</button>
            <button
              onClick={() => { if (selectedRepIds.size > 0 && date) onSave({ name: zoneName, repIds: [...selectedRepIds], date }); }}
...
function PinLegend({
  ...
  const allPinTypes = [
    ...KNOCK_RESULT_KEYS.map((k) => ({ id: k as string, name: KNOCK_LABELS[k].replace(/^[^ ]+ /, ''), color: KNOCK_COLORS[k], isCustom: false })),
  ...
  return (
    <div className="absolute bottom-24 left-3 z-10 select-none">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-gray-200 dark:border-slate-700 overflow-hidden w-52">
        {/* Header */}
        <button
          onClick={() => setExpanded((v) => !v)}
          className="w-full flex items-center justify-between px-3 py-2.5 border-b border-gray-100 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-slate-800 transition"
        >
        ...
        {expanded && (
          <>
            ...
            <div className="py-1.5">
              {allPinTypes.map((pt) => (
                <div key={pt.id} className="flex items-center justify-between px-3 py-1.5 hover:bg-gray-50 dark:hover:bg-slate-800 transition group">
            ...
            <div className={`px-3 pt-1 border-t border-gray-100 dark:border-slate-800 ${showNewPin ? '' : 'pb-2.5'}`}>
              {quickPinMode && activeType ? (
                ...
                >
                  Tap map to drop pin
                  <button onClick={onClearQuickPin} className="block w-full mt-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 font-normal text-xs">Cancel</button>
                </div>
              ) : (
                ...
                {!showNewPin ? (
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-2.5 py-1.5 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-amber-400"
                    />
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="w-8 h-7 rounded cursor-pointer border border-gray-300 dark:border-slate-600 flex-shrink-0"
                      />
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => { setShowNewPin(false); setNewPinName(''); setNewPinColor('#9333ea'); }}
                        className="flex-1 py-1.5 rounded-lg border border-gray-200 dark:border-slate-700 text-gray-500 dark:text-gray-400 text-xs hover:bg-gray-50 dark:hover:bg-slate-800 transition"
                      >
                        Cancel
                      </button>
                      <button
...
function PinActionPanel({
  ...
  const inp = 'w-full px-2.5 py-1.5 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-400';
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 sm:inset-auto sm:bottom-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-full sm:max-w-sm p-3 sm:p-0">
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] sm:max-h-[80vh] flex flex-col">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-slate-800 flex-shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                {!isQuickPin && lead.phone && (
                  <p className="text-xs text-gray-400">{lead.phone}</p>
                )}
              </div>
            </div>
            <button onClick={onClose} className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition text-gray-400 flex-shrink-0 ml-2">
              <X size={15} />
            </button>
          </div>
            ...
            <div className="px-4 py-3 space-y-3">
              ...
              {isQuickPin && (
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Street</label><input className={inp} value={form.street} onChange={(e) => sf('street', e.target.value)} placeholder="Main St" /></div>
                  <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Suburb</label><input className={inp} value={form.suburb} onChange={(e) => sf('suburb', e.target.value)} placeholder="Suburb" /></div>
                  <div><label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Postcode</label><input className={inp} value={form.postcode} onChange={(e) => sf('postcode', e.target.value)} placeholder="6000" /></div>
                </div>
              )}
          ...
          {/* Action buttons */}
          <div className="px-4 py-3 border-t border-gray-100 dark:border-slate-800 space-y-2 flex-shrink-0">
            {isQuickPin ? (
              <button
                onClick={handleSave}
              ...
              <>
                <button
                  onClick={() => { handleSave(); onClose(); onOpenProfile(); }}
                  className="w-full py-2.5 rounded-xl bg-amber-500 text-white font-semibold text-sm hover:bg-amber-400 transition"
                >
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-2 rounded-xl border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-gray-300 text-sm hover:bg-gray-50 dark:hover:bg-slate-800 transition"
                  >
                    💾 Save Only
                  </button>
                  <button
                  ...
                  <button
                    onClick={() => { onClose(); onOpenCallLogger(); }}
                    className="flex-1 py-2 rounded-xl border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-gray-300 text-sm hover:bg-gray-50 dark:hover:bg-slate-800 transition"
                  >
                    📞 Log Call
                  </button>
...
export function MapPage() {
  ...
  const { isLoaded, loadError } = useJsApiLoader({
    ...
    libraries: LIBRARIES as ('places' | 'geometry' | 'geocoding' | 'visualization')[],
  });

  const mapRef = useRef<google.maps.Map | null>(null);
  const geocoderRef = useRef<google.maps.Geocoder | null>(null);
  const clustererRef = useRef<GMClusterer | null>(null);
  // Keep a ref to imperative google.maps.Marker instances for clustering
  ...
  const getZoneColor = useCallback((repId: number) => {
  ...
  const drillLeads = useMemo(() => {
    ...
    return leads.filter(
      (l) => l.status === statDrillStatus && l.suburb === filterSuburb
    );
  }, [statDrillStatus, filterSuburb, leads]);

  // Reset drill when suburb filter changes
  useEffect(() => { setStatDrillStatus(null); }, [filterSuburb]);
  ...
  const geocodeLead = useCallback((lead: Lead) => {
    ...
    const address = `${lead.houseNum ?? ''} ${lead.street ?? ''} ${lead.suburb} ${lead.postcode ?? ''} Australia`.trim();
  ...
  const geocodeAllLeads = useCallback(async () => {
    ...
    const unmapped = geoFilterStatus
      ? allUnmapped.filter((l) => l.status === geoFilterStatus)
      : allUnmapped;
  ...
  const handleQuickPinSave = useCallback(async (latLng: google.maps.LatLngLiteral, result: string) => {
    const lead: Lead = {
  ...
  const handleKnockSave = useCallback(async (form: KnockForm & { lat: number; lng: number }) => {
    const lead: Lead = {
      id: Date.now() + Math.random(),
  ...
  const handleZoneSave = useCallback(async ({ name, repIds, date }: { name: string; repIds: number[]; date: string }) => {
  ...
  useEffect(() => {
    ...
    // Build native google.maps.Marker instances for each visible geocoded lead
    const newMarkers = visibleLeads.map((lead) => {
      ...
      const marker = new google.maps.Marker({
        ...
        title: lead.knockResult
          ? `${lead.name} — ${resolveKnockLabel(lead.knockResult, customPinTypes)}`
          : lead.name,
        icon: {
          url: markerSvgUrl(color),
    ...
    // Create or re-create the clusterer
    clustererRef.current = new GMClusterer({ map: mapRef.current, markers: newMarkers });
  ...
  const handleSaveCustomPin = useCallback(async ({ name, color }: { name: string; color: string }) => {
  ...
  const handleDeleteCustomPin = useCallback(async (id: string) => {
  ...
  const toggleFullscreen = useCallback(() => {
    if (!document.fullscreenElement) {
      ...
      (el.requestFullscreen?.() ?? Promise.reject(new Error('not supported')))
        .catch(() => showToast('Fullscreen not supported on this device', 'error'));
    } else {
  ...
  const handleOpenRoute = useCallback(() => {
    ...
    const waypoints = geocoded.slice(1, -1).map((l) => `${l.lat},${l.lng}`).join('|');
  ...
  const allUnmapped = leads.filter((l) => !l.lat || !l.lng);
  const filteredUnmapped = geoFilterStatus
    ? allUnmapped.filter((l) => l.status === geoFilterStatus)
    : allUnmapped;
    ...
    <div ref={containerRef} className="h-full w-full overflow-hidden relative">
      ...
      <div className="absolute inset-0 h-full w-full">
          ...
          <div className="absolute top-3 left-3 right-3 z-20 flex flex-col gap-2 pointer-events-none">
            ...
            <div className="flex items-stretch gap-2">
              ...
              <div className="flex items-center bg-[#111110]/90 backdrop-blur-md border border-white/[0.07] rounded-2xl shadow-2xl overflow-hidden pointer-events-auto min-w-0">
                ...
                >
                  <option value="" className="bg-[#1a1a18]">All Reps</option>
                  {activeReps.map((r) => <option key={r.id} value={r.id} className="bg-[#1a1a18]">{r.name}</option>)}
                </select>
                <div className="self-stretch w-px bg-white/[0.07]" />
                <select
                  value={filterSuburb}
                  ...
                  className="px-3 py-2.5 bg-transparent text-gray-300 text-sm outline-none cursor-pointer appearance-none"
                >
                  <option value="" className="bg-[#1a1a18]">All Suburbs</option>
                  {suburbs.map((s) => <option key={s} value={s} className="bg-[#1a1a18]">{s}</option>)}
                </select>
                <div className="self-stretch w-px bg-white/[0.07]" />
              ...
              <div className="flex items-center gap-0.5 bg-[#111110]/90 backdrop-blur-md border border-white/[0.07] rounded-2xl px-1.5 py-1.5 shadow-2xl pointer-events-auto flex-shrink-0">
                ...
                >
                  <Layers size={15} />
                  {allZones.length > 0 && (
                    <span className="w-4 h-4 bg-amber-500 text-white rounded-full text-[10px] flex items-center justify-center font-bold leading-none">{allZones.length}</span>
                  )}
                </button>
                <button
                  onClick={() => setShowHeatmap((h) => !h)}
                  title={showHeatmap ? 'Hide heatmap' : 'Show density heatmap'}
                  className={`p-2 rounded-xl transition ${showHeatmap ? 'bg-orange-500/20 text-orange-400' : 'text-gray-400 hover:bg-white/[0.06] hover:text-gray-200'}`}
                >
                  <Thermometer size={15} />
                </button>
                {showHeatmap && (
                  <div className="flex items-center gap-1 px-1">
                    ...
                    />
                    {heatmapDate && (
                      <button onClick={() => setHeatmapDate('')} className="text-gray-500 hover:text-gray-200 transition p-0.5"><X size={12} /></button>
                    )}
                  </div>
                )}
                <button
            ...
            {/* Row 2: Status chips + Knock Mode + zone drawing */}
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-0.5 bg-[#111110]/90 backdrop-blur-md border border-white/[0.07] rounded-2xl px-2 py-1.5 shadow-2xl pointer-events-auto flex-wrap">
                {ALL_STATUSES.map((s) => {
                  ...
                  return (
                    <button
                      key={s}
                      onClick={() => { setShowKnockOnly(false); toggleStatus(s); }}
                    ...
                    >
                      <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: col, opacity: active ? 1 : 0.4 }} />
                      {s}
                    </button>
                  );
                })}
              ...
              <button
                onClick={() => {
                  ...
                  if (!entering) { setKnockLatLng(null); setDrawingZone(false); setDrawVertices([]); }
                }}
                className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-semibold shadow-2xl pointer-events-auto transition ${
                  knockMode
                    ? 'bg-amber-500 text-black'
                    : 'bg-[#111110]/90 backdrop-blur-md border border-white/[0.07] text-gray-300 hover:text-white hover:bg-white/[0.05]'
                }`}
              >
                <MapPin size={14} />
              ...
              {/* Zone drawing controls — only in knock mode */}
              {knockMode && !drawingZone && (
                <button
                  onClick={() => setDrawingZone(true)}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-2xl text-sm font-medium bg-[#111110]/90 backdrop-blur-md border border-white/[0.07] text-gray-300 hover:text-amber-400 hover:border-amber-500/30 shadow-2xl pointer-events-auto transition"
                >
              ...
              {knockMode && drawingZone && (
                <>
                  <span className="text-xs font-medium text-amber-400 bg-[#111110]/90 backdrop-blur-md border border-white/[0.07] rounded-2xl px-3 py-2 shadow-2xl pointer-events-auto">
                    {drawVertices.length} pts
                  </span>
                  {drawVertices.length > 0 && (
                    <button
                      onClick={() => setDrawVertices((v) => v.slice(0, -1))}
                      className="flex items-center gap-1 px-3 py-2 rounded-2xl text-sm font-medium bg-[#111110]/90 backdrop-blur-md border border-white/[0.07] text-gray-300 hover:text-white shadow-2xl pointer-events-auto transition"
                    >
                      <Undo2 size={13} /> Undo
                    </button>
                  )}
                  {drawVertices.length >= 3 && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 rounded-2xl text-sm font-semibold bg-green-600 text-white hover:bg-green-500 shadow-2xl pointer-events-auto transition"
                    >
                      Finish Zone
                    </button>
                  )}
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-2 rounded-2xl text-sm font-medium bg-[#111110]/90 backdrop-blur-md border border-red-500/20 text-red-400 hover:text-red-300 shadow-2xl pointer-events-auto transition"
                  >
                    <X size={13} /> Cancel
                  </button>
                </>
        ...
        {!overlaysVisible && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-full shadow-xl px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 dark:hover:text-amber-400 transition"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-full shadow-xl px-3 py-2 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 transition"
            >
          ...
          {filteredZones.map((zone) => (
            <React.Fragment key={zone.id}>
              ...
              {zone.polygon.length >= 3 && (
                <OverlayView
                  position={centroid(zone.polygon)}
                ...
                >
                  ...
                  >
                    {getZoneRepNames(zone).join(', ')} · {fmtZoneDate(zone.date)}{zone.name ? ` · ${zone.name}` : ''}
                  </div>
                </OverlayView>
              )}
            </React.Fragment>
        ...
        {selectedLead && sidebarOpen && (
          <LeadSidebar
            ...
            onCall={(lead) => { setSelectedLead(lead); setShowCallLogger(true); }}
            ...
            <div className="bg-[#111110]/90 backdrop-blur-md border border-white/[0.07] rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="grid grid-cols-3">
                {([
                  ...
                  { label: 'Booked',  value: suburbStats.counts['Booked'] ?? 0,              status: 'Booked' as LeadStatus,      color: '#f59e0b' },
                  { label: 'Revisit', value: suburbStats.counts['Revisit'] ?? 0,             status: 'Revisit' as LeadStatus,     color: '#facc15' },
                  { label: 'N/I',     value: suburbStats.counts['Not Interested'] ?? 0,      status: 'Not Interested' as LeadStatus, color: '#f87171' },
                  { label: 'Conv%',   value: suburbStats.total > 0 ? `${(((suburbStats.counts['Booked'] ?? 0) / suburbStats.total) * 100).toFixed(0)}%` : '0%', status: null, color: '#4ade80' },
                ] as { label: string; value: string | number; status: LeadStatus | null; color: string }[]).map(({ label, value, status, color }) => {
        ...
        {overlaysVisible && showZoneLibrary && (
          <div className="absolute top-0 right-0 bottom-0 w-72 bg-white dark:bg-slate-900 border-l border-gray-200 dark:border-slate-700 z-20 flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-slate-700">
              <span className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <Layers size={15} className="text-amber-500" /> Zone Library
              </span>
              <button onClick={() => setShowZoneLibrary(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-slate-800 rounded transition text-gray-400"><X size={15} /></button>
            </div>
            {/* Date range filter */}
            <div className="px-4 py-3 border-b border-gray-100 dark:border-slate-800 space-y-2">
              ...
              <div className="flex gap-2 items-center">
                <input type="date" value={zoneFromDate} onChange={(e) => setZoneFromDate(e.target.value)}
                  className="flex-1 px-2 py-1.5 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white text-xs focus:outline-none focus:ring-1 focus:ring-amber-400" />
                <span className="text-gray-400 text-xs">→</span>
                <input type="date" value={zoneToDate} onChange={(e) => setZoneToDate(e.target.value)}
                  className="flex-1 px-2 py-1.5 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white text-xs focus:outline-none focus:ring-1 focus:ring-amber-400" />
              </div>
              {(zoneFromDate || zoneToDate) && (
                <button onClick={() => { setZoneFromDate(''); setZoneToDate(''); }} className="text-xs text-amber-600 dark:text-amber-400 hover:underline">Clear dates</button>
              )}
            </div>
            {/* Zone list */}
            <div className="flex-1 overflow-y-auto">
              {filteredZones.length === 0 ? (
                <p className="text-xs text-gray-400 text-center mt-8 px-4">No zones found{(zoneFromDate || zoneToDate) ? ' in date range' : ''}.<br/>Draw one in Knock Mode.</p>
              ) : (
                <ul className="divide-y divide-gray-100 dark:divide-slate-800">
                  {filteredZones.map((zone) => (
                    <li key={zone.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition group">
                      <span className="mt-1 w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: zone.color }} />
                      <div className="flex-1 min-w-0">
                        {zone.name && <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{zone.name}</p>}
                        <p className="text-xs text-gray-600 dark:text-gray-400">{getZoneRepNames(zone).join(', ') || '—'}</p>
                        <p className="text-xs text-gray-400">{fmtZoneDate(zone.date)} · {zone.polygon.length} pts</p>
                      </div>
                      <button
                        onClick={async () => { await deleteZone(zone.id); showToast('🗑️ Zone deleted', 'success'); }}
            ...
            <div className="px-4 py-3 border-t border-gray-100 dark:border-slate-800">
              <p className="text-xs text-gray-400">{filteredZones.length} / {allZones.length} zone{allZones.length !== 1 ? 's' : ''}</p>
            </div>
          </div>
        )}
        ...
        {overlaysVisible && (
          <button
            onClick={locateMe}
            className="absolute bottom-24 right-3 z-10 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl shadow-lg px-3 py-2.5 flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:border-amber-400 hover:text-amber-600 dark:hover:text-amber-400 transition"
        ...
        {overlaysVisible && unmappedCount > 0 && (
          <div className="absolute bottom-4 left-4 sm:left-4 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-3 z-10 max-w-xs">
            ...
            <div className="flex flex-wrap gap-1 mb-2">
              ...
              {(['DQ', 'Booked', 'Revisit', 'Not Interested', 'Wrong Number', 'No Answer'] as LeadStatus[])
                .map((s) => {
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-gray-300 text-xs font-medium hover:bg-gray-200 dark:hover:bg-slate-700 disabled:opacity-50 transition"
              >
                <Navigation size={12} />
      ...
      {knockLatLng && (
        <KnockModal
          ...
          onClose={() => { setKnockLatLng(null); setKnockAddress(null); }}
      ...
      {pendingZoneVertices && (
        <KnockZoneModal
          ...
          reps={activeReps}
          onClose={() => { setPendingZoneVertices(null); setDrawVertices([]); setDrawingZone(false); }}
