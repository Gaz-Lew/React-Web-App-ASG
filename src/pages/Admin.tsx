...
import { useAuditLog, useAppSettings, useSaveSettings, useLeads, useSaveRep, useDeleteRep, useServiceTypes, useSaveServiceType, useDeleteServiceType, useSaveLead, useDeleteLead } from '../hooks/useFirebase';
...
import {
  UserPlus, Pencil, Check, X, Power, ShieldCheck,
  Trash2, ChevronDown, ChevronRight, Download,
  ClipboardList, BarChart2, TrendingUp, AlertCircle,
  Settings, Phone, Eye, ChevronUp, Zap, Activity,
  CalendarCheck, PhoneCall, Users, Target, Camera, CalendarDays,
  Database, RefreshCw, Archive, Shuffle, CheckSquare, ExternalLink,
  Wifi, WifiOff, Clock, ArrowUpDown, ArrowDownCircle, ArrowUpCircle,
} from 'lucide-react';
...
function RepAvatar({
  ...
  const dims = { xs: 'w-6 h-6 text-[10px]', sm: 'w-8 h-8 text-xs', md: 'w-9 h-9 text-sm', lg: 'w-12 h-12 text-lg' }[size];
  ...
  return (
    ...
    >
      {rep.photo ? (
        ...
        <div className={`w-full h-full flex items-center justify-center ${rep.active ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400' : 'bg-gray-200 dark:bg-slate-700 text-gray-500'}`}>
          {rep.name[0]}
        </div>
      )}
...
function lastCallDate(lead: Lead): string | null {
  ...
  const dates = lead.callHistory.map((c) => c.date).filter(Boolean).sort();
...
// ── SummaryCard (shared) ───────────────────────────────────────────────────────
function SummaryCard({ label, value, sub, suffix = '' }: { label: string; value: number; sub: string; suffix?: string }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
      <div className="text-2xl font-bold text-gray-900 dark:text-white">{value}{suffix}</div>
  ...
  const cards = [
    {
      ...
      subColor: callTrend > 0 ? 'text-green-600 dark:text-green-400' : callTrend < 0 ? 'text-red-500 dark:text-red-400' : 'text-gray-400',
    },
    {
...
function LeadStatsSection({ leads }: { leads: Lead[] }) {
  ...
  const suburbCounts = useMemo(() => {
    ...
    leads.forEach((l) => { if (l.suburb) map[l.suburb] = (map[l.suburb] || 0) + 1; });
              ...
              <div key={s} className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-gray-100 dark:bg-slate-800 rounded-full h-4 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(count / maxStatusCount) * 100}%`, backgroundColor: statusColors[s] ?? DEFAULT_STATUS_COLORS[s] ?? '#9ca3af' }}
                  />
                </div>
                <span className="w-10 text-right text-xs font-semibold text-gray-900 dark:text-white flex-shrink-0">{count}</span>
            ...
            <div className="space-y-2">
              {leadsByRep.map(({ rep, count }) => (
                <div key={rep.id} className="flex items-center gap-3 text-sm">
                  ...
                  <span className="w-20 truncate text-gray-700 dark:text-gray-300 flex-shrink-0 text-xs">{rep.name}</span>
                  <div className="flex-1 bg-gray-100 dark:bg-slate-800 rounded-full h-2">
                    <div className="h-2 rounded-full bg-amber-500" style={{ width: `${(count / maxLeadCount) * 100}%` }} />
                  </div>
              ...
              {suburbCounts.map(([suburb, count], i) => (
                <div key={suburb} className="flex items-center gap-3 text-sm">
                  ...
                  <div className="w-20 bg-gray-100 dark:bg-slate-800 rounded-full h-2">
                    <div className="h-2 rounded-full bg-blue-500" style={{ width: `${(count / (suburbCounts[0]?.[1] ?? 1)) * 100}%` }} />
                  </div>
                  <span className="w-8 text-right font-semibold text-gray-900 dark:text-white text-xs">{count}</span>
                </div>
              ))}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-gray-100 dark:border-slate-800">
                  {['Name', 'Status', 'Suburb', 'Rep', 'Added'].map((h) => (
                    <th key={h} className="pb-2 text-left text-gray-500 dark:text-gray-400 font-medium pr-4">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                {recentLeads.map((l) => {
                  ...
                  const added = l.createdAt ? new Date(l.createdAt).toLocaleDateString('en-AU', { day: 'numeric', month: 'short' }) : '—';
                  return (
                    <tr key={l.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                      ...
                      <td className="py-1.5 pr-4"><StatusBadge status={l.status} /></td>
  ...
  const repStats = useMemo(() => {
    return reps.filter((r) => r.active).map((rep) => {
      ...
      const calls = leads.reduce((n, l) =>
        n + (l.callHistory?.filter((c) => c.rep === rep.name).length || 0), 0);
      ...
      const weeklyBookedActual = repLeads.filter((l) => l.status === 'Booked' && (l.bookingDate ?? '') >= startOfWeek).length;
      const monthlyBookedActual = repLeads.filter((l) => l.status === 'Booked' && (l.bookingDate ?? '') >= startOfMonth).length;

      const target: RepTarget | undefined = settings?.repTargets?.[rep.id];

      return { rep, leads: leads_count, calls, booked, revisit, conversion, target, weeklyDQActual, monthlyDQActual, weeklyBookedActual, monthlyBookedActual };
    });
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <SummaryCard label="Total Calls Logged" value={sorted.reduce((n, s) => n + s.calls, 0)} sub="all reps combined" />
        <SummaryCard label="Total Booked" value={sorted.reduce((n, s) => n + s.booked, 0)} sub="from all leads" />
        <SummaryCard label="Total Revisit" value={sorted.reduce((n, s) => n + s.revisit, 0)} sub="callbacks scheduled" />
        <SummaryCard label="Avg Conversion" value={Math.round(sorted.reduce((n, s) => n + s.conversion, 0) / Math.max(sorted.length, 1))} sub="% DQ → Booked" suffix="%" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-slate-800">
              <tr>
                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-8">#</th>
                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Rep</th>
                <SortTh label="Calls" k="calls" />
                <SortTh label="Leads" k="leads" />
                <SortTh label="Booked" k="booked" />
                <SortTh label="Revisit" k="revisit" />
                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Activity</th>
                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide whitespace-nowrap">📈 Target</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
              {sorted.map((s, i) => {
                ...
                return (
                  <tr key={s.rep.id} className={`hover:bg-gray-50 dark:hover:bg-slate-800/50 ${i < 3 ? 'bg-amber-50/30 dark:bg-amber-900/5' : ''}`}>
                    ...
                    <td className="px-3 py-3">
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">{s.booked}</span>
                    </td>
                    <td className="px-3 py-3">
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">{s.revisit}</span>
                    </td>
                    <td className="px-3 py-3 w-32">
                      <div className="flex items-center gap-1.5">
                        <div className="flex-1 h-1.5 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                          <div className="h-full rounded-full bg-amber-500" style={{ width: `${(s.calls / maxCalls) * 100}%` }} />
                        </div>
                      </div>
                    </td>
                    <td className="px-3 py-3 min-w-[140px]">
                      {(() => {
                        ...
                        if (!anyTarget) return <span className="text-xs text-gray-300 dark:text-gray-600">No target set</span>;
                        const bar = (actual: number, target: number, color: string, label: string) => (
                          <div className="flex items-center gap-1.5 text-xs">
                            ...
                            <div className="flex-1 bg-gray-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden">
                              <div className={`h-full rounded-full ${color}`} style={{ width: `${Math.min((actual / target) * 100, 100)}%` }} />
                            </div>
                            <span className={`whitespace-nowrap font-medium ${actual >= target ? 'text-green-600 dark:text-green-400' : 'text-gray-500'}`}>
                              {actual}/{target}
                            </span>
                          </div>
                        );
                        return (
                          <div className="space-y-1">
                            ...
                            {mDQ2 && bar(s.monthlyDQActual, mDQ2, 'bg-blue-400', 'Mo DQ')}
...
function OperationsSection({
  ...
  const upcoming = useMemo(() => {
    ...
    leads.forEach((l) => {
      ...
      else if (l.appointmentDate && l.appointmentDate >= today) items.push({ lead: l, date: l.appointmentDate, type: 'Appointment' });
    });
  ...
  const toggleStaleRep = (id: number) => setCollapsedStaleReps((prev) => {
  ...
  const repActivity = useMemo(() => {
    return reps.filter((r) => r.active).map((rep) => {
      ...
      if (lastCallMs >= lastDQMs && lastCallMs > 0) { recentMs = lastCallMs; recentDesc = lastCallDesc; }
      else if (lastDQMs > 0) { recentMs = lastDQMs; recentDesc = lastDQDesc; }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        ...
        <div className="divide-y divide-gray-100 dark:divide-slate-800">
          {repActivity.map(({ rep, recentMs, recentDesc }) => {
            ...
            return (
              <div key={rep.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 dark:hover:bg-slate-800/40 transition">
                {/* Avatar + pulse */}
                ...
                <div className="flex-shrink-0 text-right">
                  {recentMs > 0 ? (
                    <span className={`text-xs font-medium ${isActive ? 'text-green-600 dark:text-green-400' : isToday ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400 dark:text-gray-500'}`}>
                      {timeAgo(recentMs)}
                    </span>
                  ) : (
              ...
              <thead className="bg-gray-50 dark:bg-slate-800">
                <tr>
                  {['Rep', 'Lead Name', 'Phone', 'Suburb', 'Scheduled', 'Days Overdue'].map((h) => (
                    <th key={h} className="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                {overdueCallbacks.map(({ lead, daysOverdue }) => (
                  <tr key={lead.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                    <td className="px-3 py-2.5 text-gray-600 dark:text-gray-400 text-xs whitespace-nowrap">{getRepName(leads, reps, lead.dqRep)}</td>
                    ...
                    {items.map(({ lead, date, type }) => {
                      ...
                      return (
                        <tr key={`${lead.id}-${type}`} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                          <td className="px-3 py-2.5 text-gray-500 dark:text-gray-400 text-xs whitespace-nowrap">{date}</td>
                          <td className="px-3 py-2.5 text-xs text-gray-400">{daysAway === 0 ? 'today' : `in ${daysAway}d`}</td>
                          <td className="px-3 py-2.5 font-medium text-gray-900 dark:text-white">{lead.name}</td>
                          <td className="px-3 py-2.5 text-gray-500 dark:text-gray-400 text-xs">{getRepName(leads, reps, lead.dqRep)}</td>
                          <td className="px-3 py-2.5 text-gray-500 dark:text-gray-400 text-xs">{lead.suburb}</td>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        ...
        {staleLeads.length === 0 ? (
          <div className="py-8 text-center text-gray-400 text-sm">✅ No stale leads — all leads contacted within {staleThresholdDays} days</div>
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-slate-800">
              ...
              return (
                <div key={repId}>
                  ...
                  >
                    ...
                    <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">{rep?.name ?? `Rep #${repId}`}</span>
                    <span className="ml-auto text-xs text-gray-400">{items.length} stale {items.length === 1 ? 'lead' : 'leads'}</span>
                  </button>
                  {!collapsed && (
                    <table className="w-full text-sm">
                      <tbody className="divide-y divide-gray-50 dark:divide-slate-800/50">
                        {items.map(({ lead, lastContact, daysSince }) => (
                          <tr key={lead.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                            ...
                            <td className="px-3 py-2.5 text-xs">
                              <span className={daysSince >= 30 ? 'text-red-500 font-semibold' : 'text-amber-500 font-medium'}>
...
function SettingsSection({
  ...
  const handleSaveRates = async () => {
    ...
    if (ok) { setRatesSaved(true); setTimeout(() => setRatesSaved(false), 2000); }
  };

  const handleSaveStale = async () => {
    const ok = await onSave({ staleThresholdDays: Number(stale) });
    if (ok) { setStaleSaved(true); setTimeout(() => setStaleSaved(false), 2000); }
  };

  const handleSaveTargets = async () => {
    const ok = await onSave({ repTargets: targets });
    if (ok) { setTargetsSaved(true); setTimeout(() => setTargetsSaved(false), 2000); }
  };
  ...
  const inputCls = 'w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-400';
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
        ...
        <p className="text-xs text-gray-400 mb-4">Reference percentages shown in Comms Calculator. Informational only.</p>
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={label}>
              ...
              <input type="number" min="0" max="100" step="0.5" value={val} onChange={(e) => set(e.target.value)} className={inputCls} />
            </div>
          ))}
        </div>
        <div className="flex items-center gap-3 mt-4">
          <button onClick={handleSaveRates} className="px-4 py-2 rounded-lg bg-amber-500 text-white font-semibold text-sm hover:bg-amber-400 transition">
            Save Rates
          </button>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
        ...
        <div className="flex items-center gap-3">
          <input
            type="number" min="1" max="365" value={stale}
        ...
        <div className="flex items-center gap-3 mt-4">
          <button onClick={handleSaveStale} className="px-4 py-2 rounded-lg bg-amber-500 text-white font-semibold text-sm hover:bg-amber-400 transition">
            Save Threshold
          </button>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
        ...
        <p className="text-xs text-gray-400 mb-4">Set weekly and monthly DQ + booking targets per rep. Progress shown in Rep Performance tab.</p>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 dark:border-slate-800">
                <th className="pb-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 pr-4">Rep</th>
                <th className="pb-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 pr-3">Weekly DQ</th>
                <th className="pb-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 pr-3">Monthly DQ</th>
                <th className="pb-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 pr-3">Wkly Bookings</th>
              ...
              {activeReps.map((rep) => {
                ...
                return (
                  <tr key={rep.id}>
                    ...
                    {(['weeklyDQ', 'monthlyDQ', 'weeklyBookings', 'monthlyBookings'] as (keyof RepTarget)[]).map((field) => (
        ...
        <div className="flex items-center gap-3 mt-4">
          <button onClick={handleSaveTargets} className="px-4 py-2 rounded-lg bg-amber-500 text-white font-semibold text-sm hover:bg-amber-400 transition">
            Save All Targets
          </button>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
        ...
        <p className="text-xs text-gray-400 mb-4">Customise the colour used for each status on map pins, table badges, and charts.</p>
        ...
        <div className="flex items-center gap-3 mt-4">
          <button onClick={handleSaveColors} className="px-4 py-2 rounded-lg bg-amber-500 text-white font-semibold text-sm hover:bg-amber-400 transition">
            Save Colours
          </button>
          <button
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                rep.active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-500'
              }`}>{rep.active ? 'Active' : 'Inactive'}</span>
            </div>
            <p className="text-xs text-gray-400 mt-0.5">ID #{rep.id}</p>
          </div>
          <button onClick={onClose} className="ml-auto p-2 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800 hover:text-gray-600 transition">
            <X size={18} />
          </button>
        </div>
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                    {recentCalls.map((c, i) => (
                      <tr key={i} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                        <td className="px-3 py-2 text-gray-400 text-xs whitespace-nowrap">{c.date} {c.time}</td>
...
// ── Rep row ────────────────────────────────────────────────────────────────────
function RepRow({
  rep, onSave, onToggle, onRemove, onDrillDown,
}: {
  rep: Rep;
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      onSave({ ...rep, name: name.trim() || rep.name, role, email: email.trim() || undefined, phone: phone.trim() || undefined, abn: abn.trim() || undefined, bsb: bsb.trim() || undefined, accountNumber: accountNumber.trim() || undefined, photo: url, color: repColor.trim() || undefined });
    } catch { /* silent fail */ }
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      abn: abn.trim() || undefined, bsb: bsb.trim() || undefined, accountNumber: accountNumber.trim() || undefined,
      photo: photoUrl || undefined, pin: repPin.trim() || undefined, backupPassword: repBackupPass.trim() || undefined,
  ...
  const handleCancel = () => {
    setName(rep.name); setRole(rep.role ?? 'rep');
    setEmail(rep.email ?? ''); setPhone(rep.phone ?? '');
    setAbn(rep.abn ?? ''); setBsb(rep.bsb ?? ''); setAccountNumber(rep.accountNumber ?? '');
    setRepPin(rep.pin ?? ''); setRepBackupPass(rep.backupPassword ?? '');
    ...
    <div className={`rounded-lg border transition ${
      ...
      <div className="flex items-center gap-3 p-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500">
              ...
              {!rep.isSetup && rep.active && (
                <span className="text-amber-500 dark:text-amber-400 font-medium" title="Rep hasn't completed first-time setup yet — they need to log in and set their PIN">⚠ not set up</span>
              )}
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {editing ? (
            <>
              <button onClick={handleSave} className="p-1.5 rounded-lg bg-green-100 dark:bg-green-900/30 text-green-600 hover:bg-green-200 transition" title="Save"><Check size={14} /></button>
              <button onClick={handleCancel} className="p-1.5 rounded-lg bg-gray-100 dark:bg-slate-800 text-gray-500 hover:bg-gray-200 transition" title="Cancel"><X size={14} /></button>
            </>
          ) : (
            <>
              <button onClick={() => setExpanded(!expanded)} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800 hover:text-gray-600 transition" title="Edit details">
                {expanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
              </button>
              <button onClick={() => { setEditing(true); setExpanded(true); }} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800 hover:text-gray-600 transition" title="Quick edit name"><Pencil size={14} /></button>
              <button onClick={() => onToggle(rep.id)} className={`p-1.5 rounded-lg transition ${rep.active ? 'text-gray-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500' : 'text-gray-400 hover:bg-green-50 dark:hover:bg-green-900/20 hover:text-green-500'}`} title={rep.active ? 'Deactivate' : 'Activate'}><Power size={14} /></button>
              {confirmRemove ? (
                <>
                  <span className="text-xs text-red-500 font-medium">Sure?</span>
                  <button onClick={() => onRemove(rep.id)} className="p-1.5 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-500 hover:bg-red-100 transition" title="Confirm remove"><Check size={14} /></button>
                  <button onClick={() => setConfirmRemove(false)} className="p-1.5 rounded-lg bg-gray-100 dark:bg-slate-800 text-gray-500 hover:bg-gray-200 transition" title="Cancel"><X size={14} /></button>
                </>
              ) : (
                <button onClick={() => setConfirmRemove(true)} className="p-1.5 rounded-lg text-gray-300 dark:text-slate-600 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500 transition" title="Remove rep"><Trash2 size={14} /></button>
              )}
            </>
          )}
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-1 border-t border-gray-100 dark:border-slate-800 grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <div className="col-span-full">
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Calendar Visibility</label>
          ...
          <div className="col-span-full border-t border-gray-100 dark:border-slate-800 pt-3 mt-1">
            <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">🔐 Login Credentials</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Backup Password</label>
                <input
                  type="text"
                  value={repBackupPass}
              ...
              <div>
                ...
                <div className={`px-2 py-1.5 rounded-lg text-xs font-medium ${rep.isSetup ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400'}`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {(photoUrl || rep.photo) && (
                  <button
                    type="button"
                    onClick={() => { setPhotoUrl(''); onSave({ ...rep, photo: undefined }); }}
          ...
          <div className="col-span-full">
            ...
            <div className="p-3 rounded-lg border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50">
              {permissions.length === 0 ? (
                <p className="text-xs text-green-600 dark:text-green-400 font-medium mb-2">✓ Full access — can see all pages</p>
              ) : (
                <p className="text-xs text-amber-600 dark:text-amber-400 font-medium mb-2">⚠ Restricted — can only see selected pages</p>
              )}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                {ALL_PAGES.map(({ key, label }) => (
                  <label key={key} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg cursor-pointer select-none text-xs font-medium transition ${
                    ...
                    <input
                      ...
                      onChange={() => { if (permissions.length > 0) togglePerm(key); }}
                    />
                    {label}
                  </label>
                ))}
          ...
          {allServiceTypes.length > 0 && (
            <div className="col-span-full">
              ...
              >
                {showApptTypes ? '▾' : '▸'} Appointment Types {allowedServiceTypes.length === 0 ? '(all allowed)' : `(${allowedServiceTypes.length} selected)`}
              </button>
              {showApptTypes && (
                <div className="p-3 rounded-lg border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50">
                  <div className="flex gap-3 mb-2">
                    <button type="button" onClick={() => setAllowedServiceTypes([])} className="text-xs text-green-600 hover:underline">Allow all</button>
                    <button type="button" onClick={() => setAllowedServiceTypes(allServiceTypes.map(s => s.id))} className="text-xs text-gray-400 hover:underline">Clear all</button>
                  </div>
                  {allowedServiceTypes.length === 0 && (
                    <p className="text-xs text-green-600 dark:text-green-400 font-medium mb-2">✓ All appointment types allowed</p>
                  )}
                  {Array.from(new Set(allServiceTypes.map(s => s.category))).map(cat => (
                    <div key={cat} className="mb-2">
                      ...
                      <div className="grid grid-cols-2 gap-1">
                        {allServiceTypes.filter(s => s.category === cat).map(st => (
          ...
          <div className="col-span-full mt-1 p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50">
            <h5 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Login &amp; Access</h5>
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              {email.trim() && (
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-xs font-medium bg-blue-100 dark:bg-blue-600/20 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-600/30 hover:bg-blue-200 dark:hover:bg-blue-600/30 transition"
                >
            ...
            {!email.trim() && (
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">
                Set this rep's email in the Email field above, click Save, then use "Send Password Reset Email" to let them create their own password.
              </p>
            )}
          </div>

          <div className="col-span-full flex gap-2 pt-1">
            <button onClick={handleSave} className="px-4 py-1.5 rounded-lg bg-amber-500 text-white font-semibold text-sm hover:bg-amber-400 transition">Save</button>
            <button onClick={handleCancel} className="px-4 py-1.5 rounded-lg border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-400 text-sm hover:bg-gray-50 dark:hover:bg-slate-800 transition">Cancel</button>
          </div>
        </div>
      )}
    </div>
...
function AuditLogSection() {
  ...
  const handleExport = () => {
    exportAsCSV(
      filtered.map((e) => ({ Date: e.date, Time: e.time, User: e.user, Action: e.action, Detail: e.detail, 'Lead ID': e.leadId ?? '', 'Lead Name': e.leadName ?? '' })),
      'asg-audit-log'
    );
  };
  ...
  const actionBadge = (action: string) => {
    ...
    if (action.includes('call')) return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400';
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={handleExport} className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-700 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-800 transition">
                ...
                {filtered.map((e) => (
                  <tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-3 py-2 whitespace-nowrap">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${actionBadge(e.action)}`}>{e.action}</span>
                    </td>
...
function CalendarSettingsSection() {
  ...
  const inputCls = 'w-full px-2 py-1.5 text-sm rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-amber-500';

  // Derive categories dynamically from existing service types (+ fallback defaults)
  const defaultCatOrder = ['General', 'First Consult', 'Finance', 'Property Sale', 'Site/Lot Viewings', 'Lead Re-Engagement', 'Zoom Meeting'];
  ...
  const handleEditSave = async (st: ServiceType) => {
    await save({ ...st, name: editName.trim() || st.name, category: editCategory, color: editColor, defaultDuration: editDuration });
    setEditingId(null);
  ...
  const handleRenameCategory = async (oldName: string, newName: string) => {
    ...
    if (!trimmed || trimmed === oldName) { setEditingCatName(null); return; }
    setRenamingCat(true);
  ...
  return (
    <div className="space-y-6">
      ...
      {serviceTypes.length === 0 && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-5 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">Load the default ASG service types to get started quickly.</p>
          </div>
          <button
      ...
      {grouped.map(({ cat, types }) => (
        <div key={cat} className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
          ...
          <div className="px-5 py-3 border-b border-gray-100 dark:border-slate-800 bg-amber-50 dark:bg-amber-900/10 flex items-center gap-2">
            {editingCatName === cat ? (
              <div className="flex items-center gap-2 flex-1">
                ...
                />
                <button onClick={() => handleRenameCategory(cat, editCatValue)} disabled={renamingCat} className="text-xs px-2 py-1 bg-amber-500 text-white rounded hover:bg-amber-600 disabled:opacity-50">
                  {renamingCat ? '…' : 'Save'}
                </button>
                <button onClick={() => setEditingCatName(null)} className="text-xs px-2 py-1 text-gray-500 hover:bg-gray-100 dark:hover:bg-slate-700 rounded">✕</button>
              </div>
            ) : (
              <>
                <h3
                  ...
                  onClick={() => { setEditingCatName(cat); setEditCatValue(cat); }}
                >
                ...
                <div key={st.id}>
                  {editingId === st.id ? (
                    ...
                    <div className="px-4 py-3 bg-amber-50 dark:bg-amber-900/10 space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <input type="color" value={editColor} onChange={e => setEditColor(e.target.value)} className="h-8 w-12 rounded border border-gray-200 dark:border-slate-700 cursor-pointer" />
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleEditSave(st)} className="px-3 py-1 text-xs bg-amber-500 text-white rounded-lg hover:bg-amber-600">Save</button>
                        <button onClick={() => setEditingId(null)} className="px-3 py-1 text-xs border border-gray-200 dark:border-slate-600 text-gray-600 dark:text-gray-400 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800">Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 px-4 py-2.5">
                      ...
                      <span className="text-xs text-gray-400 dark:text-gray-500 flex-shrink-0">{st.defaultDuration}min</span>
                      {/* Action buttons */}
                      {confirmDeleteId === st.id ? (
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs text-red-500">Delete?</span>
                          <button onClick={() => handleDelete(st.id)} className="text-xs px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600">Yes</button>
                          <button onClick={() => setConfirmDeleteId(null)} className="text-xs px-2 py-1 border border-gray-200 dark:border-slate-600 rounded text-gray-500 hover:bg-gray-50 dark:hover:bg-slate-700">No</button>
                        </div>
                      ) : (
                        <div className="flex items-center gap-0.5">
                          <button onClick={() => startEdit(st)} className="p-1.5 text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded transition" title="Edit"><Pencil size={13} /></button>
                          <button onClick={() => handleCopy(st)} className="p-1.5 text-gray-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded transition" title="Duplicate">📋</button>
                          <button onClick={() => setConfirmDeleteId(st.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition" title="Delete"><Trash2 size={13} /></button>
                        </div>
                      )}
                    </div>
                  )}
...
// ── SyncSection ───────────────────────────────────────────────────────────────
function SyncSection({ settings, onSaveSettings, onOpenSyncModal }: {
  settings: AppSettings | null;
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Configure two-way sync between the CRM and a Google Sheet.</p>
      </div>
      ...
      {sync?.lastSyncAt && (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700">
          <div className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${
            sync.lastSyncResult === 'success' ? 'bg-green-500' :
            sync.lastSyncResult === 'partial' ? 'bg-amber-500' :
            sync.lastSyncResult === 'error' ? 'bg-red-500' :
            'bg-gray-400'
          }`} />
          <div className="flex-1 min-w-0">
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
            sync.lastSyncResult === 'success' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400' :
            sync.lastSyncResult === 'partial' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400' :
            sync.lastSyncResult === 'error' ? 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400' :
            'bg-gray-100 text-gray-600'
          }`}>
            {sync.lastSyncResult === 'success' ? '✓ Success' :
             sync.lastSyncResult === 'partial' ? '⚠ Partial' :
             sync.lastSyncResult === 'error' ? '✕ Error' : 'Unknown'}
          </span>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 space-y-4">
        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
          <span className="w-5 h-5 rounded-full bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center text-amber-600 dark:text-amber-400 text-xs font-bold">1</span>
          Sheet Configuration
        </h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
        ...
        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2 pt-1">
          <span className="w-5 h-5 rounded-full bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center text-amber-600 dark:text-amber-400 text-xs font-bold">2</span>
          Auto-Sync Schedule
        </h4>
      ...
      {sync?.url && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">These open the Sheets Sync modal pre-configured. You will need to authenticate with Google once per session.</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <button
              onClick={onOpenSyncModal}
              className="flex items-center justify-center gap-2 px-3 py-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400 text-sm font-medium hover:bg-blue-100 dark:hover:bg-blue-900/30 transition"
            >
      ...
      <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 rounded-xl p-4">
        <p className="text-xs font-semibold text-blue-700 dark:text-blue-400 mb-2">How Two-Way Sync Works</p>
        <ul className="text-xs text-blue-600 dark:text-blue-300 space-y-1 list-disc list-inside">
          <li><strong>Pull</strong> — reads new rows from the sheet and imports them as leads (matched by phone)</li>
          <li><strong>Push</strong> — writes all CRM leads back to the sheet (overwrites)</li>
          <li><strong>Two-Way</strong> — Pull first (get new sheet entries), then Push (export everything back)</li>
...
// ── DataToolsSection ──────────────────────────────────────────────────────────
function DataToolsSection({ leads, reps, onUpdateLeads: _onUpdateLeads, onExport }: {
  leads: Lead[];
  reps: Rep[];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select className="px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:outline-none" value={bulkFrom} onChange={(e) => { setBulkFrom(e.target.value); setConfirmBulk(false); }}>
            <option value="">From status…</option>
            {ALL_STATUSES_DT.map((s) => <option key={s} value={s}>{s} ({statusCounts[s] ?? 0})</option>)}
          </select>
          <span className="text-gray-400 text-sm">→</span>
          <select className="px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:outline-none" value={bulkTo} onChange={(e) => { setBulkTo(e.target.value); setConfirmBulk(false); }}>
            <option value="">To status…</option>
            {ALL_STATUSES_DT.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
          {bulkFrom && bulkTo && bulkFrom !== bulkTo && (
            confirmBulk ? (
              <div className="flex items-center gap-2">
                <span className="text-sm text-red-600 font-medium">Update {bulkCount} leads?</span>
                <button onClick={handleBulkStatus} className="px-3 py-1.5 rounded-lg bg-red-500 text-white text-sm font-medium hover:bg-red-400">Confirm</button>
                <button onClick={() => setConfirmBulk(false)} className="px-3 py-1.5 rounded-lg border border-gray-300 dark:border-slate-600 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700">Cancel</button>
              </div>
            ) : (
              <button onClick={() => setConfirmBulk(true)} className="px-3 py-2 rounded-lg bg-amber-500 text-white text-sm font-semibold hover:bg-amber-400">Apply to {bulkCount} leads</button>
            )
          )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 space-y-3">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400">Transfer all leads assigned to one rep to another rep.</p>
        <div className="flex flex-wrap items-center gap-2">
          <select className="px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:outline-none" value={assignFrom} onChange={(e) => { setAssignFrom(Number(e.target.value) || ''); setConfirmAssign(false); }}>
            <option value="">From rep…</option>
            {activeReps.map((r) => <option key={r.id} value={r.id}>{r.name} ({leads.filter((l) => l.dqRep === r.id).length})</option>)}
          </select>
          <span className="text-gray-400 text-sm">→</span>
          <select className="px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:outline-none" value={assignTo} onChange={(e) => { setAssignTo(Number(e.target.value) || ''); setConfirmAssign(false); }}>
            <option value="">To rep…</option>
            {activeReps.filter((r) => r.id !== assignFrom).map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}
          </select>
          {assignFrom && assignTo && (
            confirmAssign ? (
              <div className="flex items-center gap-2">
                <span className="text-sm text-red-600 font-medium">Reassign {assignCount} leads?</span>
                <button onClick={handleBulkAssign} className="px-3 py-1.5 rounded-lg bg-red-500 text-white text-sm font-medium hover:bg-red-400">Confirm</button>
                <button onClick={() => setConfirmAssign(false)} className="px-3 py-1.5 rounded-lg border border-gray-300 dark:border-slate-600 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700">Cancel</button>
              </div>
            ) : (
              <button onClick={() => setConfirmAssign(true)} className="px-3 py-2 rounded-lg bg-amber-500 text-white text-sm font-semibold hover:bg-amber-400">Reassign {assignCount} leads</button>
            )
          )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 space-y-3">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400">Permanently delete DQ leads that have never been called and were added more than N days ago.</p>
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm text-gray-700 dark:text-gray-300">Older than</span>
          <input type="number" min={7} max={365} className="w-20 px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:outline-none text-center" value={archiveDays} onChange={(e) => { setArchiveDays(Number(e.target.value)); setConfirmArchive(false); }} />
          ...
          {staleForArchive.length > 0 && (
            confirmArchive ? (
              <div className="flex items-center gap-2">
                <span className="text-sm text-red-600 font-medium">Delete {staleForArchive.length} leads permanently?</span>
                <button onClick={handleArchive} className="px-3 py-1.5 rounded-lg bg-red-500 text-white text-sm font-medium hover:bg-red-400">Delete</button>
                <button onClick={() => setConfirmArchive(false)} className="px-3 py-1.5 rounded-lg border border-gray-300 dark:border-slate-600 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700">Cancel</button>
              </div>
            ) : (
              <button onClick={() => setConfirmArchive(true)} className="px-3 py-2 rounded-lg bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 border border-red-300 dark:border-red-800 text-sm font-medium hover:bg-red-200 dark:hover:bg-red-900/50">Archive {staleForArchive.length} leads</button>
            )
          )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => onExport('leads')} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-300 text-sm font-medium hover:bg-gray-200 dark:hover:bg-slate-600 transition">
            <Download size={14} /> Export All Leads (CSV)
          </button>
          <button onClick={() => onExport('calls')} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-300 text-sm font-medium hover:bg-gray-200 dark:hover:bg-slate-600 transition">
...
export function AdminPage({ onOpenSheetsSync }: { onOpenSheetsSync?: () => void } = {}) {
  ...
  const [activeTab, setActiveTab] = useState<'reps' | 'leadstats' | 'performance' | 'operations' | 'sync' | 'data-tools' | 'settings' | 'audit' | 'calendar-settings'>('reps');
  ...
  const handleAddRep = () => {
    ...
    if (reps.some((r) => r.name.toLowerCase() === name.toLowerCase())) { setAddError('Name already exists'); return; }
    ...
    setNewName(''); setNewRole('rep'); setAddError('');
  };

  const handleAddFormerStaff = () => {
    const name = newFormerName.trim();
    if (!name) { setAddFormerError('Name is required'); return; }
    if (reps.some((r) => r.name.toLowerCase() === name.toLowerCase())) { setAddFormerError('Name already exists'); return; }
    const nextId = Math.max(0, ...reps.map((r) => r.id)) + 1;
    ...
    setReps([...reps, newRep]);
    saveRepToFirestore(newRep);
    setNewFormerName(''); setAddFormerError('');
  };
  ...
  const briefingData = useMemo((): BriefingData => {
    ...
    const apptToday = leads.filter((l) => l.bookingDate === todayDate || (l as any).appointmentDate === todayDate).length;
  ...
  const tabs = [
    ...
    { id: 'operations' as const,        label: 'Operations',       icon: <AlertCircle size={14} />, badge: overdueCount > 0 ? overdueCount : undefined },
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-slate-950 p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
            ...
            {(() => {
              ...
              return (
                <div className={`flex items-center gap-3 p-3 rounded-xl mb-4 border text-sm ${
                  withEmail === total
                  ...
                  <span>
                    <strong>{withEmail}/{total}</strong> active reps have login emails configured
            ...
            <div className="space-y-2">
              {reps.map((rep) => (
                <RepRow key={rep.id} rep={rep} onSave={handleSaveRep} onToggle={handleToggle} onRemove={handleRemove} onDrillDown={setRepDrillDown} />
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <select className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 text-sm bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-400" value={newRole} onChange={(e) => setNewRole(e.target.value as Rep['role'])}>
                    <option value="rep">Rep</option>
                    <option value="admin">Admin</option>
                  </select>
                </div>
                <button onClick={handleAddRep} className="w-full py-2 rounded-lg bg-amber-500 text-white font-semibold hover:bg-amber-400 transition text-sm flex items-center justify-center gap-2">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">
                Placeholder names for staff who no longer work here. They appear in lead-assignment dropdowns so you can import old leads without assigning them to yourself. They cannot log in.
              </p>
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Name *</label>
                  <input
                    ...
                    value={newFormerName}
                    onChange={(e) => { setNewFormerName(e.target.value); setAddFormerError(''); }}
                    onKeyDown={(e) => { if (e.key === 'Enter') handleAddFormerStaff(); }}
                  />
                  {addFormerError && <p className="text-xs text-red-500 mt-1">{addFormerError}</p>}
                </div>
                <button onClick={handleAddFormerStaff}
                ...
                <div className="space-y-2">
                  {reps
                    ...
                    .map((rep) => {
                      ...
                      return (
                        <div key={rep.id} className="flex items-center gap-3 text-sm">
                          <span className="w-16 truncate text-gray-700 dark:text-gray-300 flex-shrink-0">{rep.name}</span>
      ...
      {activeTab === 'data-tools' && (
        <DataToolsSection
          ...
          onExport={(type) => {
            ...
            reps.forEach((r) => { repsMap[r.id] = r.name; });
      ...
      {/* Rep Drill-Down Modal */}
      {repDrillDown && (
        <RepDrillDownModal rep={repDrillDown} leads={leads} onClose={() => setRepDrillDown(null)} />
      )}
    </div>
  );
}
